...
bdd_manager *bdd_new_manager(unsigned table_size, 
  ...
  bdd_manager *new_bddm = 
      (bdd_manager*) mem_get_block((SIZE_T)(sizeof (bdd_manager)));
  ...
  invariant(new_bddm->table_total_size <= BDD_MAX_TOTAL_TABLE_SIZE);
...
void bdd_kill_manager(bdd_manager *bddm) { 
  mem_free_block(bddm->node_table);
