...
unsigned ws1s___bdd_then(bdd_manager *bddm, unsigned p) {
  unsigned l, r, index;
  LOAD_lri(&bddm->node_table[p], l, r, index);
  return (r);
}

unsigned ws1s___bdd_else(bdd_manager *bddm, unsigned p) {
  unsigned l, r, index;
  LOAD_lri(&bddm->node_table[p], l, r, index);
  return (l);
}
...
if (local_##record##_primary && (local_##record##_in_use == 0)) \
  ...
  local = (struct local_##record *) \
     mem_get_block((SIZE_T)(sizeof (struct local_##record))); \
  local->a_size = 1024; \
  local->act_stack = (record*) \
      mem_get_block((SIZE_T)(sizeof (record)) * local->a_size); \
...
if (local->a == local->a_last) { \
    ...
    local->act_stack = mem_resize_block(local->act_stack,  \
...
if (local_##record##_in_use > 1) { \
  mem_free_block(local->act_stack); \
  mem_free_block(local); }; \
local_##record##_in_use--
...
#define DECLARE_A(record) \
  ...
  record *act_stack = (record*) \
      mem_get_block((SIZE_T)(sizeof (record)) * a_size); \
   record *a  = act_stack; \
...
#define INCREMENT_A \
  if (a == a_last) { \
    ...
    act_stack = mem_resize_block(act_stack,  \
