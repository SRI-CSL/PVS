...
{
   n_ssets = sz;
   ssets = mem_get_block((sizeof *ssets) * sz);
   next_sset = 0;
}
...
int make_sset(int sz, int *elem,  unsigned sq, int d1, int d2)
{
  if (next_sset == n_ssets) {
    struct set *new_ssets = mem_get_block((sizeof *ssets) * n_ssets * 2);
    
    mem_copy(new_ssets, ssets, (sizeof *new_ssets) * n_ssets);
    mem_free_block(ssets);
    ssets = new_ssets;
    n_ssets *= 2;
  }
  ...
  insert_in_hash_tab(htbl_set, (long)elem, 0, (pointer)(next_sset+1));  
...
{  
  sslist sl = mem_get_block(sizeof *sl);
...
{  
  ...
  if (state1 == state2) {
    size = 1;
    s = mem_get_block((sizeof *s) * 2);
    s[0] = state1; s[1] = -1;         
  } 
  else {
    size = 2;
    s = mem_get_block((sizeof *s) * 3);
    if(state1<state2) {
      s[0] = state1;
      s[1] = state2; 
      s[2] = -1;
  ...
  if ( (res = (int) lookup_in_hash_tab(htbl_set, (long) s, 0)) ) {
    mem_free_block(s); /* it was already there */  
    return (--res);
  }
  else {
...
{  
  ...
  s = mem_get_block((ss1->size + ss2->size + 1) * (sizeof *s));
...
DFA *dfaProject(DFA *a, unsigned var_index) 
{
  ...
  for(i = 0; i < a->ns; i++) {  /* Allocate singletons, ssets[i] = {i} */
    int *s = mem_get_block(2 * (sizeof *s));
  ...
  ssets[a->s].permanent = next_state++;  /* Should be 0 */
  {
	...
	lnxt = lst -> next;
	mem_free_block(lst);          /* Free the list */
	lst = lnxt;
      }
    
      for(i = 0; i < next_sset; i++) 
	mem_free_block(ssets[i].elements);
