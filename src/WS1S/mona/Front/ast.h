...
enum ASTKind {
  ...
  aEx2, aAll0, aAll1, aAll2, aLet0, aLet1, aLet2, aBind1, aBind2, aCall, 
  aUniv, aImport, aExport, aPrefix, aDot2, aUp2, aRootPred, aInStateSpace
};

enum ASTOrder {oTerm1, oTerm2, oForm, oUniv};

class ASTSubstElement {
public:
  Ident formal;
  class AST *actual;
};

class ASTSubst: public Deque<ASTSubstElement> {}; 
...
class ASTTerm1_Dot: public ASTTerm1 {
public:
  ASTTerm1_Dot(ASTTerm1 *tt, BitList *bts, Pos p) :
    ASTTerm1(aDot1, p), bits(bts), t(tt) {}
  ~ASTTerm1_Dot() {delete bits; delete t;}
...
class ASTTerm1_Up: public ASTTerm1 {
public:
  ASTTerm1_Up(ASTTerm1 *tt, Pos p) :
    ASTTerm1(aUp1, p), t(tt) {}
  ~ASTTerm1_Up() {delete t;}
...
protected:
  ...
  ASTForm *fold(IdentList *IdentList);
};
...
class ASTForm_Bind1: public ASTForm_nt {
public:
  ASTForm_Bind1(int n, ASTTerm1 *t, Pos p) :
    ASTForm_nt(aBind1, n, t, p) {}

  VarCode makeCode(SubstCode *subst = NULL);
  void dump() {};
};

class ASTForm_Bind2: public ASTForm_nT {
public:
  ASTForm_Bind2(int n, ASTTerm2 *T, Pos p) :
    ASTForm_nT(aBind2, n, T, p) {}
...
class ASTForm_Export: public ASTForm {
public:
  ASTForm_Export(ASTForm *ff, char* fil, Pos p) :
    ASTForm(aExport, p), f(ff) , file(fil) {}
  ~ASTForm_Export() {delete f;}
...
class ASTForm_Prefix: public ASTForm {
public:
  ASTForm_Prefix(ASTForm *ff, Pos p) :
    ASTForm(aPrefix, p), f(ff) {}
  ~ASTForm_Prefix() {delete f;}
...
class ASTForm_InStateSpace: public ASTForm {
public:
  ASTForm_InStateSpace(ASTTerm1 *tt, IdentList *s, Pos p) :
    ASTForm(aInStateSpace, p), t(tt), ss(s) {}
  ~ASTForm_InStateSpace() {delete t; delete ss;}
...
public:
  MonaAST(ASTForm *f, ASTForm *a) :
    formula(f), assertion(a), lastPosVar(-1) {}
