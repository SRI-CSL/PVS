...
(defun pp-extension (extension)
  (typecase (car extension)
    (field-decl
     (pprint-logical-block (nil extension :prefix "[# " :suffix " #]")
       (loop (pp* (pprint-pop))
	     (pprint-exit-if-list-exhausted)
	     (write-char #\,)
	     (write-char #\space)
...
(defmethod pp* ((ex unary-application))
  (with-slots (operator argument) ex
	...
	(pprint-logical-block (nil nil)
	  (pprint-indent :current 2)
	  (write (id operator))
	  (when (string-equal (id operator) "NOT")
	    (write-char #\space)
	    (pprint-newline :miser))
