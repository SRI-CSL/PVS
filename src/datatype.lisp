...
(defmethod generate-adt-subtypes ((adt recursive-type-with-subtypes))
  ...
  (let ((*generate-tccs* 'none))
	...
	(let* ((djsubtype (mk-expr-as-type (mk-name-expr (recognizer c))))
	       (jsubtype (typecheck* djsubtype nil nil nil))
	       (jtype (typecheck* (subtype c) nil nil nil)))
	  (unless (subtype-of? jsubtype jtype)
	    (let ((jdecl (make-instance 'subtype-judgement
			   :declared-subtype (mk-expr-as-type (mk-name-expr (recognizer c)))
      ...
      (let* ((tdom (typecheck* (copy domain) nil nil nil))
	     ...
	     (subst-range (substit range
				...
				(pairlis
				 ...
				 (cons dtype
				       (mapcar #'(lambda (dep)
						   (let ((app (mk-application (id dep) dep-name))
							 (*generate-tccs* 'none))
						     (typecheck* app (type dep) nil nil)))
					 deps)))
				(pairlis
...
(defmethod acc-reduce-selection (arg (te type-name) red fname fdom adt)
  (acc-reduce-selection-name arg te red fname fdom adt))

(defun acc-reduce-selection-name (arg te red fname fdom adt)
  (cond ((subtype-of? te adt)
	 (mk-application red (copy arg)))
	((adt? (find-adt-supertype te))
	 (let* ((adt-te (find-adt-supertype te))
		(acts (actuals-corresponding-to-positive-types
		       (actuals (module-instance adt-te))
		       (adt? adt-te)))
		...
		(dacts (dactuals-corresponding-to-positive-types
			(dactuals (module-instance adt-te))
			(adt? adt-te)))
	   ...
	   (if (every #'identity-fun? funs)
		   ...
		   (let ((appname (mk-name-expr '|map|
				    (lift-actuals-for-map (actuals adt-te) adt)
				    (id (module-instance adt-te)))))
		     (setf (dactuals appname) (append (dactuals adt-te) dfacts))
		     (let ((pname (pc-parse (unparse appname :string t) 'expr)))
		       (mk-application (mk-application* pname dfuns)
			 (copy arg)))))
	       (if (every #'identity-fun? dfuns)
		   (let* ((appname (mk-name-expr '|map|
				     (append (lift-actuals-for-map (actuals adt-te) adt)
					     facts)))
...
(defun lift-actuals-for-map (acts adt)
  (mapcar #'(lambda (act) (lift-actual-for-map act adt)) acts))

(defun lift-actual-for-map (act adt)
  (assert (actual? act))
  (assert (adt-type-name? adt))
  (if (and (type-value act)
	   (subtype-of? (type-value act) adt))
      (mk-actual adt)
      act))
...
(defmethod acc-reduce-selection (arg (te subtype) red fname fdom adt)
  (if (finite-set-type? te)
	...
	(if (identity-fun? fun)
		...
		(copy arg)))))
      (let* ((suptype (find-adt-supertype te))
	     (ndom (if (typep fdom '(and subtype (not datatype-subtype)))
		       (supertype fdom)
		       fdom)))
	(typecase suptype
	  ((or funtype recordtype tupletype cotupletype)
	   (acc-reduce-selection arg suptype red fname ndom adt))
	  (type-name (acc-reduce-selection-name arg suptype red fname ndom adt))
	  (t (break "Need case"))))))
...
(defmethod acc-subterm-selection* ((te type-name) xvar adtname adt)
  (acc-subterm-selection-name te xvar adtname adt))

(defun acc-subterm-selection-name (te xvar adtname adt)
  (cond ((subtype-of? te adtname)
	...
	((adt? (find-adt-supertype te))
	 (let ((subs (mapcar #'(lambda (act)
				 (acc-subterm-selection* (type-value act)
							 xvar adtname adt))
		       (positive-actuals (find-adt-supertype te)))))
	   (if (every #'everywhere-false? subs)
	       (mk-everywhere-false-function te)
	       (mk-predicate-application '|some| te adt subs))))
	(t (mk-everywhere-false-function te))))
...
(defmethod acc-subterm-selection* ((te subtype) xvar adtname adt)
  (if (finite-set-type? te)
	...
	(typecase suptype
	  ((or funtype recordtype tupletype cotupletype)
	   (acc-subterm-selection* suptype xvar adtname adt))
	  (type-name (acc-subterm-selection-name te xvar adtname adt))
	  (t (break "Need case"))))))
...
(defmethod acc-<<-selection* ((te type-name) xvar adtname adt)
  (acc-<<-selection-name te xvar adtname adt))

(defun acc-<<-selection-name (te xvar adtname adt)
  (cond ((subtype-of? te adtname)
	 (let ((bd (mk-bind-decl (make-new-variable '|z| te) te)))
	   (mk-lambda-expr (list bd)
...
(defmethod acc-<<-selection* ((te subtype) xvar adtname adt)
  (if (finite-set-type? te)
	...
	(typecase suptype
	  ((or funtype recordtype tupletype cotupletype)
	   (acc-<<-selection* suptype xvar adtname adt))
	  (type-name (acc-<<-selection-name te xvar adtname adt))
	  (t (break "Need case"))))))
