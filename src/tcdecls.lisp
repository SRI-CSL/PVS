...
(defmethod typecheck* ((ue units-appl) expected kind arguments)
  ...
  (case (operator ue)
    ...
    (^ (let ((lhs (if (numberp (args1 ue))
	 ...
	 (if (or (numberp lhs)
		 (not (numberp rhs)))
	     (type-error ue "Expt must be of form 'unitname^number'")
	     (setf (scale ue) (expt (scale lhs) rhs)
		   (dimensionality ue) (dimensionality lhs)))
	 ue))
    (t (break))))
...
(defmethod typecheck* ((decl const-decl) expected kind arguments)
  ...
  (let* ((*bound-variables* (apply #'append (formals decl)))
    ...
    (set-type (declared-type decl) nil)
    (setf (units decl)
	  (when (and (name? (declared-type decl))
		     (units-decl? (declaration (declared-type decl))))
	    (declared-units (declaration (declared-type decl)))))
    (setf (type decl)
	...
	(typecheck* (definition decl) rtype (units decl) nil))
