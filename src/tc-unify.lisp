...
(defmethod tc-match* :around ((arg type-expr) farg bindings)
  (if (tc-eq arg farg)
	...
	(if (and nbindings
		 (print-type arg)
		 (or (and (typep (print-type arg) 'name)
			  (actuals (module-instance (print-type arg)))
			  (eq (module (declaration (print-type arg)))
			      *formals-theory*))
		     (and (typep (print-type arg) 'type-application)
			  (actuals (module-instance (type (print-type arg))))
			  (eq (module (declaration (type (print-type arg))))
			      *formals-theory*))))
...
(defun completed-tc-match-bindings (nbind)
  (when (every #'cdr (if (equal (car nbind) *strict-tc-match*)
			 (cdr nbind)
			 nbind))
    nbind))
