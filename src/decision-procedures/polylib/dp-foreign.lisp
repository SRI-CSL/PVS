...
(defun domain-eq (p1 p2)
  (= (domain-address p1) (domain-address p2)))

(defun domain= (p1 num)
  (= (domain-address p1) num))
...
(defun domain-finalize! (p)
  (excl:schedule-finalization p #'domain_free))
...
(defun polyhedron-next (domain)
  (mk-domain
   (poly::polyhedron-next (domain-address domain))))
...
(defun constraints2polyhedron (constraints maxrays)
  (mk-domain
   (poly::constraints2polyhedron constraints maxrays)))

(defun domainaddconstraints (Pol1 Mat2 NbMaxRays)
  (mk-domain
   (poly::domainaddconstraints (domain-address Pol1) Mat2 NbMaxRays)))

(defun mydomainaddconstraints (Pol1 Mat2 NbMaxRays)
  (mk-domain
   (poly::mydomainaddconstraints (domain-address Pol1) Mat2 NbMaxRays)))

(defun domainaddrays (Pol1 Mat2 NbMaxRays)
  (mk-domain
   (poly::domainaddrays (domain-address Pol1) Mat2 NbMaxRays)))
...
(defun domaindifference (pol1 pol2 maxrays)
  (mk-domain
   (poly::domaindifference (domain-address pol1)
			  (domain-address pol2) maxrays)))

(defun domainimage (pol1 func maxrays)
  (mk-domain
   (poly::domainimage (domain-address pol1) func maxrays)))

(defun domainintersection (pol1 pol2 maxrays)
  (mk-domain
   (poly::domainintersection (domain-address pol1)
			     (domain-address pol2) maxrays)))

(defun domainpreimage (pol1 func maxrays)
  (mk-domain
   (poly::domainpreimage (domain-address pol1) func maxrays)))

(defun domainsimplify (pol1 pol2 maxrays)
  (mk-domain
   (poly::domainsimplify (domain-address pol1) (domain-address pol2) maxrays)))

(defun domainunion (pol1 pol2 maxrays)
  (mk-domain
   (poly::domainunion (domain-address pol1) (domain-address pol2) maxrays)))

(defun domain_copy (pol)
  (mk-domain
   (poly::domain_copy (domain-address pol))))
...
(defun mypolyhedronincludes (pol1 pol2)
...
(defun mydomainincludes (pol1 pol2)
...
(defun rays2polyhedron (ray maxrays)
  (mk-domain
   (poly::rays2polyhedron ray maxrays)))

(defun subconstraint (Con2 Pol1 NbMaxRays Pass)
  (mk-domain
   (poly::subconstraint con2 (domain-address pol1)
			NbMaxRays Pass)))

(defun universe_polyhedron (dimension)
  (mk-domain
   (poly::universe_polyhedron dimension)))
...
(defun matrix_set (matrix row col val)
  (poly::matrix_set matrix row col val))

(defun matrix_ref (matrix row col)
