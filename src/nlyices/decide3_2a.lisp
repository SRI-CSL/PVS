...
;;
;; The returned value is a list of polynomials G' that has the same
;; properties as G. 
;;
;; NOTES: 
...
(defun grobner (E G)
  ...
  (grobner-core (grobner-preprocess E) nil G))
...
;; Result:
;; - G' = a basis of (union E M G)
;;
(defun grobner-core (E M G)
  (print-debug 3 t "grobner core E: ~a polys, M: ~a polys, G: ~a polys~%" (length E) (length M) (length G))
  (if (null M) 
      (if (null E) G
	  (let ((newE (sort-by-length E)))
	    (grobner-core (cdr newE) (grobner-simplify (car newE) G) G)))
    ...
    (let ((p (car M)))
	  ...
	  (list p) ;; the basis is reduced to p
	(multiple-value-bind 
	 (E1 CP G1) (grobner-add-poly G p)
	 (grobner-core (nconc E E1 CP) (cdr M) G1))))))
...
(defun sos-full-tst (E G) 
  ...
  (let ((G1 (grobner E G)))
	...
	(if wit 
	   ...
	   (values t G1))))))   ;; no unsat witness found
