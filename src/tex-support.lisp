...
;; Last Modified On: Sun Apr 30 19:15:49 1995
;; Update Count    : 9
;; Status          : Beta test
;; 
;; HISTORY
...
\\documentstyle[fullpage,fleqn,12pt,supertabular,~a/pvs]{article}

\\begin{document}
% Uncomment the following line to get line numbers in your documents.
%\\linenumstrue
~{\\input{~a}~%~}
...
(defun latex-proof (texfile &optional terse? show-subst)
  (if *last-proof*
	...
	(cond ((stringp texfile)
	       (with-open-file (stream texfile :direction :output
				       :if-exists :supersede)
