...
;; Added @DECL as a keyword, but generated lex is not case-sensitive,
;; so need to add sensitivity
(defun LEX-@ (stream symbol)
  (declare (ignore symbol))
  (let (holdchar)
    (setf holdchar (lexical-read-char stream :eof))
    (if (and PVS-ESCAPE-CHAR (eql holdchar PVS-ESCAPE-CHAR))
        (setf holdchar (lexical-read-char stream :eof)))
    (cond ((eql holdchar #\@) 'SBST::@@)
          ((char-equal holdchar #\D)
           (setf holdchar (lexical-read-char stream :eof))
           (if (and PVS-ESCAPE-CHAR (eql holdchar PVS-ESCAPE-CHAR))
               (setf holdchar (lexical-read-char stream :eof)))
           (cond ((char-equal holdchar #\E)
                  (setf holdchar (lexical-read-char stream :eof))
                  (if (and PVS-ESCAPE-CHAR
                           (eql holdchar PVS-ESCAPE-CHAR))
                      (setf holdchar (lexical-read-char stream :eof)))
                  (cond ((char-equal holdchar #\C)
                         (setf holdchar
                               (lexical-read-char stream :eof))
                         (if (and PVS-ESCAPE-CHAR
                                  (eql holdchar PVS-ESCAPE-CHAR))
                             (setf holdchar
                                   (lexical-read-char stream :eof)))
                         (cond ((char-equal holdchar #\L) 'SBST::@DECL)
                               (t
                                (lexical-unread-char stream)
                                (illegal-token-error "@DEC")
                                :illegal-token)))
                        (t
                         (lexical-unread-char stream)
                         (illegal-token-error "@DE")
                         :illegal-token)))
                 (t
                  (lexical-unread-char stream)
                  (illegal-token-error "@D")
                  :illegal-token)))
          (t (lexical-unread-char stream) 'SBST::@))))
