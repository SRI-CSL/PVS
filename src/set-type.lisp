...
(defun set-type-name-expr (ex expected creses)
  ...
  (let ((res (find-best-name-resolution ex creses expected)))
    ...
    (unless (compatible? (type res) expected)
      (if (integerp (id ex))
	  (change-class ex 'number-expr 'number (id ex) 'type *real*)
...
(defmethod set-type-actuals ((modinst modname))
  ...
  (let ((nmodinst (simplify-modinst modinst)))
	...
	(generate-mapped-axiom-tccs nmodinst modinst))
...
(defmethod set-type-actual (act (formal formal-theory-decl))
  ...
  (let ((threses (remove-if-not
		   ...
		   (resolutions (expr act)))))
    (cond ((cdr threses)
	   (type-error "Theory name ~a is ambiguous" (expr act)))
	  ((null threses)
	   (type-error "No resolution for theory name ~a" (expr act)))
	  (t (setf (resolutions (expr act)) threses)))
    (set-type-actuals (expr act))))

(defun set-type-mappings (mappings modinst &optional map-bindings)
  (when mappings
    (let ((lhs (lhs (car mappings)))
      ...
      (set-type-mapping-rhs rhs lhs modinst map-bindings)
      (set-type-mappings (cdr mappings) modinst
			 (acons (declaration lhs) rhs map-bindings))
      )))

(defun set-type-mapping-rhs (rhs lhs modinst map-bindings)
  (cond ((type-decl? (declaration lhs))
	...
	(t (let* ((ptypes (remove-if-not #'type-expr? (ptypes (expr rhs))))
		  (types (get-compatible-rhs-types
			  ...
			  ptypes rhs modinst map-bindings)))
	     (cond ((null types)
...
(defun get-compatible-rhs-types (lhs-type rhs-types rhs modinst map-bindings)
  (let* ((*subst-mod-params-cache*
	  (make-hash-table :hash-function 'pvs-sxhash :test 'strong-tc-eq))
	 (subst-type (subst-mod-params* lhs-type modinst map-bindings)))
...
(defun set-conditional-arg-types (op arg1 arg2)
  (case op
    ((and & implies =>)
     ...
     (let ((*tcc-conditions* (append (nreverse (and!+ arg1))
				     *tcc-conditions*)))
    ...
    ((when)
     (set-type* arg2 *boolean*)
     (let ((*tcc-conditions* (append (nreverse (and!+ arg2))
				     *tcc-conditions*)))
       (set-type* arg1 *boolean*)))))
...
(defun set-type-application-arguments (expr args argument
  ...
  (cond ((or (conjunction? expr)
	 ...
	 (let ((*tcc-conditions* (append (nreverse (and!+ (car args)))
					 *tcc-conditions*)))
	...
	((boolean-when-expr? expr)
	 (set-type* (cadr args) *boolean*)
	 (let ((*tcc-conditions* (append (nreverse (and!+ (cadr args)))
					 *tcc-conditions*)))
	   (set-type* (car args) *boolean*)
	   (setf (type argument) (mk-tupletype (mapcar #'type args)))
	   (set-expr-type expr expected)))
  ...
  (let* ((op (operator ex))
	...
	(if reses
	  ...
	  (let ((econd (condition ex))
	    ...
	    (let ((*tcc-conditions* (append (nreverse (and!+ econd))
					    *tcc-conditions*)))
