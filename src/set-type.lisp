...
(defmethod set-type-actuals ((modinst modname))
  (let ((fmls (formals-sans-usings (get-theory modinst)))
    ...
    (set-type-mappings (mappings modinst)))
...
(defun set-type-mappings (mappings)
  (when mappings
    (let ((lhs (lhs (car mappings)))
      ...
      (set-type* rhs lhs)
      (set-type-mappings (cdr mappings))
      )))

(defmethod set-type* ((rhs mapping-rhs) lhs)
  (if (type-decl? (declaration lhs))
      (if (type-value rhs)
	  (set-type* (type-value rhs) nil)
	  (type-error (expr rhs) "Type expected here"))
      (let ((types (remove-if-not #'type-expr? (ptypes (expr rhs)))))
	(cond ((null types)
