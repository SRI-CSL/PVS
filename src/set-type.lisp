...
(defmethod set-type* ((ex application) expected)
  (with-slots (operator argument type) ex
    (let* ((types (types ex))
	  ...
	  (let ((optype (determine-operator-type operator argument expected)))
...
(defun set-conditional-arg-types (op arg1 arg2)
  (case op
    ((and & implies =>)
     ...
     (let ((*tcc-conditions* (append (nreverse (and+ arg1)) *tcc-conditions*)))
    ...
    ((when)
     (set-type* arg2 *boolean*)
     (let ((*tcc-conditions* (append (nreverse (and+ arg2)) *tcc-conditions*)))
       (set-type* arg1 *boolean*)))))
  ...
  (if (type operator)
	...
	(let* ((optypes1 (if (cdr coptypes)
	       ...
	       (optypes5 (if (cdr optypes4)
			     ...
			     optypes4))
	       (optypes (if (cdr optypes5)
			    (explicit-importings operator optypes5)
			    optypes5)))
	  (assert optypes)
