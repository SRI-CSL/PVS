...
(defun make-function-copy-info (type-name-root) ;; c-domain-root
  (let* ((name (format nil "copy_~a" type-name-root))
	 (header (format nil "~a_t ~a(~a_t x)" type-name-root name type-name-root
			 ;;(mppointer-type c-domain-root)
			 ))
    ...
    (with-open-file (output file-string :direction :output
		    ...
		    (loop for thy in  preceding-prelude-theories
			  ...
			  (when (ht-instance-clone thy)
			    (maphash #'(lambda (x y)(format output  "~%~%#include \"~a_c.h\"" x))
		    ...
		    (loop for thy in preceding-prelude-theories
			  do (when (ht-instance-clone thy)
			       (maphash #'(lambda (x y)(format output  " ~a_c.c" x))
					(ht-instance-clone thy))))
		    (loop for thy in preceding-theories
...
(defparameter *primitive-prelude-theories*
  '(booleans equalities notequal if_def boolean_props quantifier_props
