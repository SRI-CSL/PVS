...
(defmethod cl2pvs* (sexpr (type funtype) context)
  ...
  (let* ((ub (simple-below? (domtype type)))
	 (bv-cnv (when ub (find '|bv| (current-conversions) :key #'id))))
    (if (and bv-cnv
	     (tc-eq (range type) *boolean*)
	     ;;(from-prelude? bv-cnv)
	     )
	;; We have a bitvector and a bv conversion - create a form like bv(0b11001010)
	(typecase sexpr
	  (array (let ((num 0))
		   (dotimes (i (number ub))
		     (when (aref sexpr i)
		       (incf num (expt 2 (- (number ub) i 1)))))
		   (tc-expr (format nil "bv[~a](0b~b)" ub num))))
	  (function (let ((num 0))
		      (dotimes (i (number ub))
			(when (funcall sexpr i)
			  (incf num (expt 2 (- (number ub) i 1)))))
		      (tc-expr (format nil "bv[~a](0b~b)" ub num))))
	  (pvs-outer-array (let ((num 0))
			     (dotimes (i (number ub))
			       (when (pvs-outer-array-lookup sexpr i)
				 (incf num (expt 2 (- (number ub) i 1)))))
			     (tc-expr (format nil "bv[~a](0b~b)" ub num))))
	  (t (break "not a closure or array")))
