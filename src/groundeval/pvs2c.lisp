  ...
  (with-slots
	...
	(let* ((ir-function-name (ir-fname ir-function-name))
	       (pre-ir (if ir-args
			   ...
			   ir-defn))
	       (post-ir (preprocess-ir pre-ir ir-return-type))
...
(defun print-header-file (theory-id theory)
  (let* ((file-string (if *pvs2c-library-path*
    ...
    (unless  *pvs2c-library-path*; (or  (file-exists-p "Makefile")) overwriting Makefile
      (uiop::copy-file (format nil "~a/src/groundeval/pvs2c-Makefile" *pvs-path*)
		       "pvs2c/Makefile")
      (replace-in-file "<pvspath>" (format nil "~a" *pvs-path*) "pvs2c/Makefile"))
    ...
    (let* ((theory-id (id theory))
      ...
      (cond ((null test-formulas)
	    ...
	    (t (with-open-file (main filename :direction :output :if-exists :supersede)
		 (format main "// test main ~a generated by make-c-tests-main; To run tests, invoke \"make run_~a\" in the pvs2c subdirectory of the current workspace." theory-id theory-id)
		 ...
		 (format t "~%Wrote ~a. To run tests, invoke \"make run_~a\" in the pvs2c subdirectory of the current workspace" filename theory-id))
		 ...
		 (with-open-file (output mkfile-string :direction :output
		   ...
		   (format output "~%OBJS_~a := $(SRCDIR)/~a.o $(SRCDIR)/~a_c.o ~{~a_c.o ~} ~{~a_c.o ~}"
			   ...
			   (loop for thy in  *all-mono-theories* collect (format nil "$(SRCDIR)/~a" (id thy))))
		   (format output "~%bin/~a: $(OBJS_~a) | $(BINDIR)~%~c$(CC) $(CFLAGS) $(LDFLAGS) -o $@ $^ $(LDLIBS)" theory-id theory-id #\Tab)
		   (format output "~%~%.PHONY: ~a" theory-id)
		   (format output "~%~a: bin/~a" theory-id theory-id)
		   (format output "~%~%.PHONY: run_~a" theory-id)
		   (format output "~%run_~a: ~a~%~c./bin/~a~%"  theory-id theory-id #\Tab theory-id)
