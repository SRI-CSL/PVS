...
(defun prettyprint-theory-instance (instance &optional importing-theory)
  (let ((*current-theory* (if importing-theory
			      (get-theory importing-theory)
			      (car (last *prelude-theories*)))))
    (if *current-theory*
	(let* ((*current-context* (context *current-theory*))
	       (*generate-tccs* 'none)
	       (inst (if (stringp instance)
			 (pc-parse instance 'modname)
			 (get-name-at (car instance) (cdr instance)))))
	  (when (stringp instance)
	    (typecheck-using inst))
	  (let ((subst-th (create-theory-instance inst))
		(*unparse-expanded* t))
	    (pvs-buffer (format nil "~a.instance" (id inst))
	      (unparse subst-th :string t)
	      t t)))
	(pvs-message "Theory ~a not found" importing-theory))))

(defun create-theory-instance (inst)
  (let* ((atheory (get-theory inst))
	 (theory (if (recursive-type? atheory)
		     (adt-theory atheory)
		     atheory)))
    (if (or (actuals inst)
	    (mappings inst))
	(let ((bindings (make-subst-mod-params-map-bindings
			 inst (mappings inst) nil)))
	  (make-instance 'module
	    'id (id inst)
	    'formals nil
	    'assuming (create-theory-instance* (assuming theory) inst bindings)
	    'theory (create-theory-instance* (theory theory) inst bindings)
	    'exporting (make-instance 'exporting 'kind 'DEFAULT)
	    ))
	theory)))

(defmethod create-theory-instance* ((list list) inst bindings)
  (create-theory-instance-list list inst bindings))

(defun create-theory-instance-list (list inst bindings &optional result)
  (if (null list)
      (nreverse result)
      (let ((elt (create-theory-instance* (car list) inst bindings)))
	(create-theory-instance-list
	 (cdr list) inst bindings
	 (if elt
	     (cons elt result)
	     result)))))
(defmethod create-theory-instance* ((decl type-decl) inst bindings)
  (unless (assq decl bindings)
    decl))

(defmethod create-theory-instance* ((decl type-def-decl) inst bindings)
  (lcopy decl
    'type-expr (subst-mod-params (type-expr decl) inst)
    'contains (subst-mod-params (contains decl) inst)))

(defmethod create-theory-instance* ((decl const-decl) inst bindings)
  (unless (assq decl bindings)
    (lcopy decl
      'formals (subst-mod-params (formals decl) inst)
      'declared-type (subst-mod-params (declared-type decl) inst)
      'definition (subst-mod-params (definition decl) inst))))

(defmethod create-theory-instance* ((decl def-decl) inst bindings)
  (lcopy decl
    'formals (subst-mod-params (formals decl) inst)
    'declared-type (subst-mod-params (declared-type decl) inst)
    'definition (subst-mod-params (definition decl) inst)
    'declared-measure (subst-mod-params (declared-measure decl) inst)
    'ordering (subst-mod-params (ordering decl) inst)))

(defmethod create-theory-instance* ((decl formula-decl) inst bindings)
  (lcopy decl
    'formals (subst-mod-params (formals decl) inst)
    'definition (subst-mod-params (definition decl) inst)))
