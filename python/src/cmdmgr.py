...
def execute(callback, command, *parameters, **keywords):
    ...
    runner.manager.sendAsyncCommand(callback, command, *parameters, **keywords)
...
def typecheck(filename):
    ...
    if runner.manager == None or runner.manager.status != PVS_MODE_EDIT:
...
def onTypecheck(request, result):
    ...
    name = request[runner.PVSRunner.PARAMETERS][0]
...
def onGetTheories(request, result):
    ...
    filename = request[FULLNAME]
    gui.manager.addTheoriesToFileTree(filename, result)
...
def onProverStarted(request, result):
    ...
    if result.has_key[INPROVER] and result[INPROVER]==True:
        runner.manager.setStatus(PVS_MODE_PROVER)
