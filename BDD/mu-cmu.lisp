...
;;; run-musimp moved to mu-cmu-load.lisp because of the
;;; way cmu lisp works with foreign functions in saved images.
...
(defmethod convert-pvs-to-mu* ((expr expr));;NSH(8.17.95) added find-supertype
  (cond ((tc-eq expr *true*) (mu-mk-true))
	...
	((recordtype? (find-supertype (type expr)))
	 (convert-pvs-to-mu*
...
(defun make-subrange-names (expr &optional type)
  (let* ((type (or type (type expr)))
	 ...
	 (bddvarid (if bddvarid-hash
		       (car bddvarid-hash)
		       (make-bdd-var-id)))
	 (bddvarid-list (if bddvarid-hash
			    (cadr bddvarid-hash)
			    (loop for index from 0 to (1- len)
				  collect
				  (make-bdd-incl-excl-var-id
				   bddvarid index)))))
...
(defun subrange!? (type)
  (let* ((subrange (type-value (declaration (subrange-res))))
         (*modsubst* t)
         (subst (match subrange type nil nil)))
