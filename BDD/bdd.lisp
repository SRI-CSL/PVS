...
(ff:defforeign 'null_list :entry-point '"bdd___null_list")
;;; #define ELEM_CONTENTS(elem) ((elem)->cont)
(ff:defforeign 'elem_contents :entry-point '"bdd___elem_contents")
;;; #define LIST_FIRST(list) ((list)->start_p)
(ff:defforeign 'list_first :entry-point '"bdd___list_first")
;;; #define LIST_LAST(list)  ((list)->end_p)
(ff:defforeign 'list_last :entry-point '"bdd___list_last")
;;; #define LIST_INFO(list)  ((list)->info)
(ff:defforeign 'list_info :entry-point '"bdd___list_info")
;;; #define LIST_NEXT(elem)  ((elem)->next)
(ff:defforeign 'list_next :entry-point '"bdd___list_next")
...
(ff:defforeign 'set_bdd_do_gc :entry-point '"bdd___set_bdd_do_gc")
;;; int bdd_do_dynamic_ordering;/* default 1 */
(ff:defforeign 'set_bdd_do_dynamic_ordering :entry-point '"bdd___set_bdd_do_dynamic_ordering")
;;; int bdd_verbose;            /* default 0 */
(ff:defforeign 'set_bdd_verbose :entry-point '"bdd___set_bdd_verbose")
;;; int bdd_use_neg_edges;      /* default 1*/
(ff:defforeign 'set_bdd_use_neg_edges :entry-point '"bdd___set_bdd_use_neg_edges")
;;; int bdd_use_inv_edges;      /* default 1; 0 when bdd_do_dynamic_ordering = 1 */
(ff:defforeign 'set_bdd_use_inv_edges :entry-point '"bdd___set_bdd_use_inv_edges")
...
;;; BDD_VARID (F)
(ff:defforeign 'bdd_varid :entry-point '"bdd___bdd_varid")
;;; BDD_THEN (F)
;;; BDD_ELSE (F)
;;; BDD_REFCOUNT (F)
;;; BDD_FLAG (F)
...
;;; BDD_VOID_P (f)
(ff:defforeign 'bdd_void_p :entry-point '"bdd___bdd_void_p")
;;; BDD_1_P (f)
(ff:defforeign 'bdd_1_p :entry-point '"bdd___bdd_1_p")
;;; BDD_0_P (f)
(ff:defforeign 'bdd_0_p :entry-point '"bdd___bdd_0_p")
;;; BDD_X_P (f)
(ff:defforeign 'bdd_x_p :entry-point '"bdd___bdd_x_p")
;;; BDD_CONST_P (f)
(ff:defforeign 'bdd_const_p :entry-point '"bdd___bdd_const_p")
;;; BDD_TERM_P (f)
...
;;; BDD_COFACTOR_NEG (f)
(ff:defforeign 'bdd_cofactor_neg_ :entry-point '"bdd___bdd_cofactor_neg_")
...
(ff:defforeign 'bdd_size :entry-point '"bdd___bdd_size")

;;; int bdd_size_vec (BDDPTR *f_vec, int size)
;;; int bdd_size_ceil (BDDPTR f, int ceiling)

;;; void bdd_init (void)
(ff:defforeign 'bdd_init :entry-point '"bdd___bdd_init")
;;; void bdd_free (BDDPTR f)
(ff:defforeign 'bdd_free :entry-point '"bdd___bdd_free")
;;; int bdd_gc (void)
(ff:defforeign 'bdd_gc :entry-point '"bdd___bdd_gc")
;;; BDDPTR bdd_ite (BDDPTR F, BDDPTR G, BDDPTR H)
(ff:defforeign 'bdd_ite :entry-point '"bdd___bdd_ite")
;;; BDDPTR bdd_ite_const (BDDPTR F, BDDPTR G, BDDPTR H)
(ff:defforeign 'bdd_ite_const :entry-point '"bdd___bdd_ite_const")
;;; void bdd_cofactors (BDDPTR f, BDDPTR *vp, BDDPTR *Tp, BDDPTR *Ep)
;;; BDDPTR bdd_invert_input_top (BDDPTR f)
(ff:defforeign 'bdd_invert_input_top :entry-point '"bdd___bdd_invert_input_top")
;;; BDDPTR bdd_create_var (int v)
(ff:defforeign 'bdd_create_var :entry-point '"bdd___bdd_create_var")
;;; BDDPTR bdd_create_var_first	(void)
(ff:defforeign 'bdd_create_var_first :entry-point '"bdd___bdd_create_var_first")

(ff:defforeign 'bdd_create_var_before :entry-point '"bdd___bdd_create_var_before")
;;; BDDPTR bdd_create_var_after	(BDDPTR v)
(ff:defforeign 'bdd_create_var_after :entry-point '"bdd___bdd_create_var_after")
;;; BDDPTR bdd_create_var_last	(void)
(ff:defforeign 'bdd_create_var_last :entry-point '"bdd___bdd_create_var_last")
;;; void bdd_print (FILE *fp, BDDPTR f, char *s)
(ff:defforeign 'bdd_print :entry-point '"bdd___bdd_print")
;;; void bdd_print_stats (FILE *fp)
;;; void bdd_quit (void)
(ff:defforeign 'bdd_quit :entry-point '"bdd___bdd_quit")
...
(ff:defforeign 'bdd_nodes_alive :entry-point '"bdd___bdd_nodes_alive")
;;; int bdd_nodes_allocated (void)
(ff:defforeign 'bdd_nodes_allocated :entry-point '"bdd___bdd_nodes_allocated")
...
(ff:defforeign 'bdd_and :entry-point '"bdd___bdd_and")
;;; BDDPTR bdd_greater	(BDDPTR F, BDDPTR G)
(ff:defforeign 'bdd_greater :entry-point '"bdd___bdd_greater")
;;; BDDPTR bdd_less (BDDPTR F, BDDPTR G)
(ff:defforeign 'bdd_less :entry-point '"bdd___bdd_less")
;;; BDDPTR bdd_xor (BDDPTR F, BDDPTR G)
(ff:defforeign 'bdd_xor :entry-point '"bdd___bdd_xor")
;;; BDDPTR bdd_or (BDDPTR F, BDDPTR G)
(ff:defforeign 'bdd_or :entry-point '"bdd___bdd_or")
;;; BDDPTR bdd_nor (BDDPTR F, BDDPTR G)
(ff:defforeign 'bdd_nor :entry-point '"bdd___bdd_nor")
;;; BDDPTR bdd_equiv (BDDPTR F, BDDPTR G)
(ff:defforeign 'bdd_equiv :entry-point '"bdd___bdd_equiv")
;;; BDDPTR bdd_xnor (BDDPTR F, BDDPTR G) /* equivalent to bdd_equiv */
(ff:defforeign 'bdd_xnor :entry-point '"bdd___bdd_xnor")
;;; BDDPTR bdd_implied (BDDPTR F, BDDPTR G)
(ff:defforeign 'bdd_implied :entry-point '"bdd___bdd_implied")
;;; BDDPTR bdd_implies (BDDPTR F, BDDPTR G)
(ff:defforeign 'bdd_implies :entry-point '"bdd___bdd_implies")
;;; BDDPTR bdd_nand (BDDPTR F, BDDPTR G)
(ff:defforeign 'bdd_nand :entry-point '"bdd___bdd_nand")
;;; BDDPTR bdd_0 (void)
(ff:defforeign 'bdd_0 :entry-point '"bdd___bdd_0")
;;; BDDPTR bdd_1 (void)
(ff:defforeign 'bdd_1 :entry-point '"bdd___bdd_1")
;;; BDDPTR bdd_X (void)
(ff:defforeign 'bdd_X :entry-point '"bdd___bdd_X")
;;; BDDPTR bdd_assign (BDDPTR f)
(ff:defforeign 'bdd_assign :entry-point '"bdd___bdd_assign")
;;; BDDPTR bdd_top_var (BDDPTR f)
;;; int bdd_top_var_rank (BDDPTR f)
;;; BDDPTR bdd_then (BDDPTR f)
(ff:defforeign 'bdd_then :entry-point '"bdd___bdd_then")
