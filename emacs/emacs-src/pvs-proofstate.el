...
(defun pvs-proofstate-no-frame (&optional reset)
  (let* ((frame-names-alist (make-frame-names-alist))
...
(defun pvs-proofstate-1-frame (&optional reset)
  (let* ((frame-names-alist (make-frame-names-alist))
	 (pvs-frame (or (cdr (assoc "pvs" frame-names-alist))
			;; Unlikely, but just take the first one if no pvs
			(cdar frame-names-alist)))
...
(defun pvs-proofstate-2-frame (&optional reset)
  (let* ((frame-names-alist (make-frame-names-alist))
	 (pvs-frame (or (cdr (assoc "pvs" frame-names-alist))
			;; Unlikely, but just take the first one if no pvs
			(cdar frame-names-alist)))
