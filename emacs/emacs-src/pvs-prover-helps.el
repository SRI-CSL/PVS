...
(if (memq pvs-emacs-system '(xemacs20 xemacs19))
    (progn
	...
	(if (re-search-backward "^{[0-9]*}" (point-min) t)
		...
		(setq found-expansion
		      (and 
		       ...
		       (y-or-n-p (concat "Expand " def2expand " ")))))
	      (goto-char cpt)))
      (setq def2expand (read-from-minibuffer "Expand: ")))
  ...
  (let ((editprfbuf (get-buffer "Proof")))
    (save-excursion
      ...
      (cond ((memq pvs-emacs-system '(emacs20 emacs19))
	    ...
	    ((memq pvs-emacs-system '(xemacs20 xemacs19))
	     (let ((beg (point))
		   (end (save-excursion (forward-sexp 1) (point))))
