...
(defun list-changes-lisp ()
  ...
  (let ((names (reverse (mapcar (function
				 (lambda (change)
				  (with-current-buffer (marker-buffer change)
				    (goto-char change)
				    (lisp-def-name))))
				lisp-changes))))
    (if names
	(with-output-to-temp-buffer "*Changed-Definitions*"
	  ...
	  (with-current-buffer "*Changed-Definitions*"
	    (goto-char (point-min))
	    (kill-line)
...
list and passing it on to the normal error handler." 
  (let ((change (car ilisp-pending-changes)))
    (when (and comint-errorp
	       (not (cl-member change lisp-changes
			     :test #'equal
			     :key #'marker-position)))
  ...
  (with-current-buffer (ilisp-buffer)
	...
	(let* ((change (car changes))
      ...
      (comint-send-code
       (ilisp-process)
       (function (lambda ()
	 (with-current-buffer (get-buffer-create "*Last-Changes*")
	   (delete-region (point-min) (point-max))
	   (insert (with-current-buffer "*Errors*"
		     (buffer-string)))))))
      (if keep
