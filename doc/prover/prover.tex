...
\usepackage{relsize,alltt,makeidx,url,boxedminipage,fancyheadings,/project/pvs/pvs2.0/pvs}
\usepackage{../makebnf}
...
\newcommand{\indbang}{{\tt
induct-and-rewrite!}\index{induct-and-rewrite"!@{\texttt{induct-and-rewrite"!}}}}
...
%\input{/homes/csla/rushby/tex/prelude}
...
been that such attempted proofs of \emph{putative theorems\index{putative
theorems}} very quickly highlight even subtle errors and infelicities.
...
For example, the powerful Boyer-Moore theorem prover\index{Nqthm}~\cite{Boyer-Moore88} uses a constructive, untyped,
quantifier-free first order logic as its specification language.  On the
other hand, ``proof assistants'' for Z\index{Z} and VDM\index{VDM} (such
as {\sl Mural}\index{mural@{\sl Mural}}~\cite{VDM-in-mural}) provide attractive mouse-and-windows
...
These proof obligations, termed \emph{type correctness conditions} (or TCCs\index{TCC}), arise
...
proof checker (such as LCF\index{LCF}~\cite{LCF},
...
Otter\index{Otter}~\cite{Otter90} or 
Nqthm\index{Nqthm}~\cite{Boyer-Moore79,Boyer-Moore88}).  Compared with
thoroughly automated theorem provers such as Nqthm\index{Nqthm}, \pvs's
deductive component may be considered a proof checker---but it seems like
a theorem prover to those accustomed to systems such as HOL\index{HOL}
...
IMPLY\index{IMPLY} prover of Bledsoe\index{Bledsoe, Woodrow W.} and his
...
At any time in a \pvs\ proof, attention is focussed on some sequent
that is a leaf node in the current proof tree---this is the sequent
that is displayed by the \pvs\ prover while awaiting the user's
command.  The numbers in brackets, \eg\ \texttt{[-3]}, and braces, \eg\ {\tt
\{3\}},  before each formula in
the displayed sequent are used to name the corresponding formulas.
The formula numbers in square brackets (\eg\ \texttt{[-3]} above)
indicate formulas that are unchanged in a subgoal from the parent goal
whereas the 
numbers in braces (\eg\ \{2\} in the example
above), serve to highlight those formulas that are either new or
different from those of the parent sequent.
...
The following paragraphs summarizes how proof attempts are initiated,
...
A proof session is initiated from within an Emacs buffer containing
a PVS specification by using the Emacs command \emacstt{pr} with the
cursor at the formula that is to be proved.  
If the formula has already been proved, the user is asked whether
the proof attempt should proceed.  If a proof or partial proof for the
relevant formula already exists, then the user is asked if
this proof should be rerun.  The Emacs command \emacstt{xpr} can be used
to initiate a proof that generates  a Tcl/Tk display of the proof
structure as it is being developed.
...
remaining text is generated by \pvs\@.  A proof attempt can be abandoned
by typing \texttt{q} or \indtt{quit} at the prompt.  At the end of a
successful or abandoned proof attempt, the user is queried as to
whether the resulting partial proof should be saved.  The timing
characteristics are displayed at the end.  The saved partial proof can be
rerun in a subsequent attempt so that the unfinished parts of the proof
can be completed.  
...
page~\pageref{help} for more details. 

\prparagraph{Interrupting Proofs}{Interrupting Proofs}

The proof checker can be interrupted when it is working on a command
by typing \indtt{C-c C-c}.  This places the system at a Lisp break.
Typing \texttt{(restore)} at the break returns the system to the \texttt{Rule?}\ 
...
Once the proof is initiated\footnote{See the previous chapter or the System
Guide~\cite{PVS:userguide} for details on how to initiate proof attempts.}, the main goal is displayed
in the \texttt{*pvs*} buffer followed by a \texttt{Rule?}\ prompt.
The user commands are typed in at this prompt.  The first command,
\skobang{}, introduces Skolem constants \texttt{f!1}, \texttt{g!1}, and {\tt
n!1} for the universally quantified variables in the theorem.
The second command, \indtt{lemma}, 
introduces the induction scheme for natural numbers
\texttt{nat\_induction}\index{nat\_induction@{\texttt{nat\_induction}}} as an antecedent formula.  This induction scheme
is proved as a lemma in the theory \indtt{naturalnumbers} in the \pvs\
prelude.\footnote{The lemma name can also be given in its full
form \texttt{naturalnumbers.nat\_induction} if the theory name is
needed for disambiguation.  See the \pvs\ language manual~\cite{PVS:language} for more details 
of how name resolution is performed.}
...
Apply the conjunctive splitting command \indtt{split} to the goal
yields three subgoals.  The first goal is to demonstrate that the
conclusion of the instantiated induction scheme implies the
original conjecture following the introduction of Skolem constants.
The second subgoal is the base case, and the third subgoal
is the induction step.  The first subgoal is easily proved by using the
heuristic instantiation command \indtt{inst?}\@.
...
numbered \texttt{1} which is easily proved by expanding the definition of
\texttt{sum} using the \indtt{expand} command.  Notice that this command
uses the PVS decision procedures to simplify the definition of \texttt{sum}
and to reduce the equality to \texttt{TRUE}\@.
...
\end{smallsession}
If we apply the \indtt{expand} command selectively to
expand those occurrences of \texttt{sum} on the consequent side,
we get a sequent that is tautologously true.  Notice, once again,
that the expand command makes significant use of type information within
the PVS decision procedures in order to simplify not only the
expanded definition of \texttt{sum} but also the resulting equality
...
and wallclock time for the proof attempt are displayed above.

\section{The Example Proof Redone}

PVS has a language in which proof strategies can be written.
The essence of the above proof can actually be captured as a
strategy.  The strategy \indbang{} invokes
induction according to the scheme appropriate to the given
induction variable and then completes the proof by expanding the
functions used in the theorem and applying heuristic instantiation
...
resulting subgoals interactively using further proof commands, or
to backtrack (using \indtt{undo}) in order to  try alternative proof
commands.  When a strategy is invoked with a \texttt{\char36} suffix,
e.g., \texttt{induct-and-rewrite!\char36}\index{induct-and-rewrite"!@{\tt
induct-and-rewrite"!\char36}}, the strategy is executed so that the
expanded internal steps are visible.  This mode of strategy invocation
provides more information and is useful when debugging.  


The Emacs command \emacstt{show-last-proof} can be
used to get a summary of the most recently completed proof.
Note that the Emacs commands \emacstt{add-declaration}
and \emacstt{modify-declaration} can be used to alter the specification
even while a proof is progress.  Since these commands can affect the
validity of a proof so  proofs should always be rerun in order to check
their validity.   During the course of a proof, the Emacs command
\emacstt{ancestry} displays the sequence of goals leading back to the
root goal of the proof, \emacstt{siblings} displays the sibling subgoals of
the current goal, \emacstt{show-hidden} displays the hidden formulas of the
current sequent, and \emacstt{show-auto-rewrites} displays those rewrite
...
$\alpha$, there is an \indtt{IF} operation with the signature $[{\tt
bool}, \alpha, \alpha \rightarrow \alpha]$.    The
...
The sequent calculus inference rules displayed in Chapter~\ref{logic}
for the basis for the proof commands used to construct proofs with \pvs\@.
The \pvs\ proof commands are however significantly more powerful than
these simple inference rules so as to make proof construction process
more illuminating and less tedious. 
Proof commands can be typed in by the user at the \texttt{Rule?}\ prompt or
they can be automatically applied by \pvs\ as part of a proof strategy.
A  PVS proof command  when applied to a goal sequent either 
\begin{enumerate}
...
command that signals failure.  Failure is used to either backtrack or
...
\begin{enumerate}\raggedright
  ...
  \item Quantifier rules:
    ...
    \item Existential: \indtt{inst}, \indtt{instantiate},
    \indtt{instantiate-one}, \indtt{inst-cp}, \indtt{inst?}, and
    \indtt{quant?}.

    \item Universal: \indtt{detuple-boundvars}, \indtt{skolem},
    \indtt{skolem-typepred}, \skobang{}, \indtt{skosimp},
    \indtt{skosimp*}, \indtt{generalize}, and
    \indtt{generalize-skolem-constants}.

    \end{enumerate}

  \item Equality rules: \indtt{beta}, \indtt{case-replace}, \indtt{name},
  \indtt{name-replace}, \indtt{name-replace*}, \indtt{replace},
  \indtt{replace*}, and \indtt{same-name}.

  \item Rules for using definitions and lemmas:
    \begin{enumerate}
      \item Definition expansion: \indtt{expand}, and \indtt{expand*}.
      \item Using lemmas: \indtt{forward-chain}, \indtt{lemma}, \indtt{use} and
    \indtt{use*}.  
...
\indtt{assert}, \indtt{bash}, \indtt{both-sides}, \indtt{do-rewrite},
\indtt{ground}, \indtt{grind}, \indtt{record}, \indtt{reduce},
\indtt{simplify}, \indtt{simplify-with-rewrites} and \indtt{smash}.
...
\indtt{stop-rewrite}, and \indtt{stop-rewrite-theory}.
  ...
  \item Making type constraints explicit: \indtt{typepred}, and
\indttbang{typepred}.

  \item Model Checking: \indtt{model-check}, and \indtt{musimp}. 
  ...
  \indtt{spread}, \indtt{spread!}, \indtt{spread@}, \indtt{then},
  \indtt{then*}, \indtt{then@}, \indtt{time}, \indtt{try}, and
  \indtt{try-branch}
...
\indtt{prop} is the atomic propositional simplification rule and {\tt
prop\char36} is the corresponding strategy.

In pragmatic terms, strategies should be used when the expanded proof is
of interest and  otherwise, rules should be used.  So for instance, {\tt
prop} should almost always be favored over \texttt{prop\char36} since the
...
\indtt{help} & \emph{primitive} & provides brief documentation\\\hline
\end{tabular}

\prsubsection{help}{Get help for proof commands}
\begin{description}
\item[syntax:] \texttt{(help \optl\ \emph{name\default{*}})}

\item[effect:] Displays a brief description of a specific primitive
proof command, defined
rule, or strategy, or of all of the rules, defined rules, and
strategies.  Apart from displaying help information, the \indtt{help} rule
behaves as a \texttt{(skip)}\indextt{skip}, \ie\ it has no effect on the proof.
...
\emacstt{help-pvs-prover-command}, \emacstt{help-pvs-prover-strategy}, or
\emacstt{help-pvs-prover-emacs}, since they display the information in a
buffer for repeated reference.  The Emacs commands
...
\begin{tabular}{|l|l|l|}\hline
\indtt{comment} & \emph{primitive} & attach comments to proof
sequents\\\hline
\indtt{label} & \emph{primitive} & attach labels to sequent
formulas\\\hline
\indtt{unlabel} & \emph{primitive} & removes labels from sequent
formulas\\\hline
\indtt{with-labels} & \emph{defined} & labels resulting formulas \\\hline
\end{tabular}

\prsubsection{comment}{Attach Comments to a Proof Sequent}
\begin{description}
\item[syntax:] \texttt{(comment\ \emph{string})}

\item[effect:] Attaches a comment string to the current proof sequent
that is printed with preceding semi-colons above the sequent
formulas.  This comment string is also saved with the proof.
The \texttt{comment} command can be nested within strategies and the comments
are retained on the subgoals generated by the strategy.  

\begin{usage}{}
\item[\texttt{(comment "3rd induction case")} :] Prints~the comment string
\texttt{;;;3rd induction case} between the sequent label and the
sequent formulas.  
\end{usage}

\end{description}

\prsubsection{label}{Attaches a Label to Sequent Formulas}
\begin{description}
\item[syntax:] \texttt{(label\ \emph{string-or-symbol} \emph{fnums}
\optl\ \emph{push?})}

\item[effect:]  It is often useful to group and label
a collection of related formulas in a proof sequent.  The \indtt{label}
command is used for this purpose.  Each sequent formula can bear
at most one label.  The label is printed alongside the \emph{fnum}
whenever a proof sequent is displayed.  A label can be used wherever an
\emph{fnum} is expected.  A label can supplied as either a string,
e.g., \texttt{"label" } or a symbol, e.g., \texttt{|label|},
though it is stored internally  as a symbol.   Labels are
automatically inherited by any subformulas of a sequent formulas
that appear through the application of an inference rule, e.g.,
\texttt{flatten} applied to a consequent formula $A \vee B$ labelled \texttt{main}
results in two sequent formulas $A$ and $B$ both labelled \texttt{main}.

When \emph{push?} is \texttt{t}, the new label is added to any existing labels
on the formula.  Otherwise, the given label replaces any existing ones.

\begin{usage}{}
\item[\texttt{(label "uniqueness" -3)} :] Labels the formula numbered {\tt
-3} by the label \texttt{uniqueness}.

\item[\texttt{(label "type-constraints" (-1 -3 -4))} :] Labels the formulas
numbered \texttt{-1}, \texttt{-3}, and \texttt{-4} by the label \texttt{type-constraints}.

\item[\texttt{(label "antecedents" - :push? t)} :] Adds the label
\texttt{type-constraints} in addition to any other labels on
the formulas numbered \texttt{-1}, \texttt{-3}, and \texttt{-4}.

\item[\texttt{(bddsimp "type-constraints" )} :] Applies BDD-based
propositional simplification to the formulas labelled {\tt
type-constraints}\@.
\end{usage}
...
\item[notes:] 
Note that the \texttt{bddsimp} command does not retain labels since
there is no simple way to retain the connection between the formula
returned by BDD-simplification and its original parent formula.
\end{description}
\prsubsection{unlabel}{Removes Labels from Sequent Formulas}
\begin{description}
\item[syntax:] \texttt{(unlabel\ \optl\ \emph{fnums})}

\item[effect:]  Removes all labels from the formulas in
\emph{fnums}.  When \emph{fnums} is not specified, all
labels from all formulas are removed.

\end{description}
\index{Proof Rules!Annotation|)}


\prsubsection{with-labels}{Labels New Sequent Formulas}
\begin{description}
\item[syntax:] \texttt{(with-labels\ \emph{rule} \emph{labels} \optl\ \emph{push?})}
...
\begin{description}
\item[syntax:]  \texttt{(postpone \optl\ \emph{print?})}

\item[effect:] Marks the current goal as pending to be proved and shifts
the focus to the next remaining goal.  By successively invoking {\tt
postpone} sufficiently often, it is possible to cycle back to the
...
rerunning it when the proof attempts is resumed.
...
\item[effect:] In the default mode, automatic rewriting by
commands such as {\rm assert} and {\rm do-rewrite}
generate a fairly verbose commentary.  This can be
entirely shut off by the {\rm rewrite-msg-off} command.
Behaves like a {\rm skip} otherwise. 

\item[notes:] Finer-grain control over the terseness of the
rewriting commentary can be obtained by the Emacs commands
...
\item[effect:] The rewriting commentary turned off by {\rm
rewrite-msg-off} can be restored by this command.  Behaves like a {\rm
skip} otherwise.  
\end{description}
...
\item[syntax:] \texttt{(set-print-depth num)}

\item[effect:] Sets the print depth for displaying formulas.  Num must be
a number.  0 means print the entire formula, any other number causes terms
below the given depth to be elided.  Behaves like a {\rm skip} otherwise.
\end{description}

\prsubsection{set-print-length}{Set the Print Length}
\begin{description}

\item[syntax:] \texttt{(set-print-length num)}

\item[effect:] Sets the print length for displaying formulas.  Num must be
a number.  0 means print the entire formula, any other number causes terms
longet than the given number to be elided.  Behaves like a {\rm skip}
otherwise.
\end{description}
...
\item[syntax:] \texttt{(set-print-lines num)}

\item[effect:] Sets the number of print lines for displaying formulas.
Num must be a number.  0 means print the entire formula, any other number
causes only the first num lines of each formula of the sequent to be
displayed.  Behaves like a {\rm skip} otherwise.
\end{description}

\prsubsection{skip}{A Do Nothing Rule}
\begin{description}
\item[syntax:] \texttt{(skip)}

\item[effect:] Has no effect on the proof.  The primary utility of {\tt
skip} is in writing strategies where a step is required to have no
...
\prsubsection{skip-msg}{A Do Nothing, Print Something Rule}
\begin{description}
\item[syntax:] \texttt{(skip-msg \emph{string} \optl\ \emph{force-printing?})}

\item[effect:] Has no effect on the proof but prints the given {\em
string}.  The main use of \texttt{skip-msg} is in generating error
messages from within strategies, typically as: \texttt{(if good?(input)
\ldots (skip-msg "Bad input."))}\@.

\begin{usage}{}
\item[\texttt{(skip-msg "No such theory in current context.")}]: Does
nothing but prints the error message \texttt{"No such theory in current
context."}\@.

\item[\texttt{(skip-msg "No such theory in current context." :force-printing?
T)}]: Does nothing but prints the error message \texttt{"No such theory in current
context."} even when the \texttt{skip-msg} appear within an \texttt{apply} where
the printing of such messages is usually suppressed.

\item[]\begin{alltt}
(let ((string
         (format nil "No such theory: ~a in current context."
                                                     theory)))
   (skip-msg string))
...
\prsubsection{trace}{Trace Given Commands}
\begin{description}
\item[syntax:] \texttt{(trace \rest\ \emph{names})}

\item[effect:] Turns on the tracing of the proof commands named in {\em
names} so that any time any one of the named rules or strategies is used
...
invoked and has the desired effect.   Behaves like a {\rm skip} otherwise.
...
\prsubsection{track-rewrite}{Explains Failure to Apply Rewrite Rule }
\begin{description}
\item[syntax:] \texttt{(track-rewrite \rest\ \emph{names})}
...
simplification, \texttt{track-rewrite} behaves like a {\rm skip}.  It has
...
\begin{description}
\item[syntax:] \texttt{(undo \optl\ \emph{to\default{1}})}
...
labelled \texttt{assoc.2}, unless \texttt{assoc.2} labels the current node in
...
\prsubsection{untrace}{Disables Tracing of Given Commands}
\begin{description}
\item[syntax:] \texttt{(untrace \rest\ \emph{names})}
...
\prsubsection{untrack-rewrite}{Disables Tracking of Rewrite Rules}
\begin{description}
\item[syntax:] \texttt{(untrack-rewrite \rest\ \emph{names})}
...
{\rm skip}.  It has no effect on the current proof sequent and is not
...
\begin{tabular}{|l|l|l|}\hline
\indttdol{copy} & \emph{defined} & inserts a copy of a sequent formula \\\hline
\indtt{delete} & \emph{primitive} &  deletes selected formulas from a goal sequent\\\hline
\indtt{hide} & \emph{primitive} & temporarily hides selected formulas from the displayed goal \\\hline
\indtt{reveal} & \emph{primitive} & reveals hidden formulas\\\hline
\end{tabular}

Sequent calculus based proof systems employ structural rules to
rearrange the formulas in a sequent.  The typical structural rules are
described in Chapter~\ref{logic}.  In \pvs, the Exchange rule is entirely
omitted since the \pvs\ proof commands already ignore the
order of formula occurrences in a sequent except for the use of formula
numbers.  The Contraction rule is not built into \pvs\ and only
appears in a limited form; the rule for instantiating quantifiers
of existential strength permits the copying of these quantified formulas
so that they can be reused, if needed.  Some use of Contraction is
already built into the rules so that the non-principal formulas are
shared between the premises of a rule.  The defined rule \indtt{copy}
also implements Contraction.  
The Weakening rule is present in
\pvs\ as the \indtt{delete} rule below.  The \indtt{hide} rule is a more
cautious form of \indtt{delete}, where certain sequent formulas can be
suppressed and recovered later in the proof using the \indtt{reveal} rule.
The emacs command \emacstt{show-hidden-formulas}  displays  hidden
formulas along with their numbers.

\prdolsubsection{copy}{Copy Selected Formula}
\begin{description}
\item[syntax:]  \texttt{(copy \emph{fnum})}
...
\begin{description}
\item[syntax:]  \texttt{(delete \rest\ \emph{fnums})}

\item[effect:] Returns the subgoal that is the result of deleting all
of the sequent formulas in the current goal that are indicated by {\em
fnums}.  If there are no formulas in the sequent corresponding
...
\begin{description}
\item[syntax:] \texttt{(hide \rest\ \emph{fnums})}

\item[effect:] This is a more cautious version of \indtt{delete}.  The {\tt
hide} rule saves the deleted sequent formulas that are indicated by {\em
fnums} so that they can be restored to a descendant of the current sequent
...
\prdolsubsection{hide-all-but}{Hide Selected Formulas}
\begin{description}
\item[syntax:] \texttt{(hide-all-but\ \optl\ \emph{keep-fnums} \emph{fnums[*]})}

\item[effect:] This is a variant of the \indtt{hide} rule that hides all
the formulas indicated by \emph{fnums} except those indicated by {\em
keep-fnums}\@.  As with \indtt{hide}, hidden sequent formulas are saved
...
\prsubsection{reveal}{Reveal Some Hidden Formulas}
\begin{description}
\item[syntax:] \texttt{(reveal \rest\ \emph{fnums})}

\item[effect:]  To see the hidden formulas in sequent form, use
the emacs command \emacstt{show-hidden}.   Invoking \indtt{reveal} yields
a subgoal that reintroduces the hidden formulas numbered \emph{fnums}
(in the sequent displayed by \emacstt{show-hidden}) into the current
sequent.   The formulas thus revealed are not removed from the list
of hidden formulas.

\begin{usage}{}
\item[\texttt{(reveal -2)}] : Reveals the formula numbered \texttt{-2} in
the sequent displayed by \emacstt{show-hidden}.

\item[\texttt{(reveal (-1 4 -3 2))}] : Reveals the formulas numbered {\tt
-1}, \texttt{4}, \texttt{-3}, \texttt{2} in the sequent displayed by
\emacstt{show-hidden}.
...
\prsubsection{bddsimp}{Propositional Simplification}
\begin{description}

\item[syntax:] \texttt{(bddsimp \optl\ \emph{fnums\default{*}} \emph{dynamic-ordering?} \emph{irredundant?\default{t}})}
...
\begin{description}
\item[syntax:] \texttt{(case \rest\ \emph{exprs})}
...
\end{eqnarray*}
Note that the \texttt{case} command generates
$n+1$ subgoals given $n$ formulas.  
This allows us to assume a formula or a collection of formulas and
subsequently prove these formulas to be true.  The formulas $A_i$ are
...
additional subgoals corresponding to the type correctness conditions
\eg\ \texttt{(case "(1/x) > 0")} would generate an additional subgoal
with the proof obligation requiring that \texttt{x /= 0}.
It is quite common for this command to generate parser and typechecker
errors which simply return control back to the proof checker without
affecting the state of the proof.

This command is extremely useful for transforming an undesirable
expression $t$ into a more desirable form $s$ when  this cannot be
achieved by the other proof commands.  The command \texttt{(case "$t$ = $s$")}
followed by \indtt{replace} can achieve the desired transformation.
This combination is encapsulated in the defined rule \indtt{case-replace}
...
additional branches if the typechecking of the formula in place of {\tt
x > 0} generates type correctness proof obligations.
...
\begin{description}
\item[syntax:] \texttt{(flatten \rest\ \emph{fnums\default{*}})}
...
\item[\texttt{(flatten 2)}] : disjunctively simplifies formula number {\tt
2} in the current goal sequent.
...
\begin{description}
\item[syntax:] \texttt{(flatten-disjunct \optl\ \emph{fnums depth})}
...
\prsubsection{iff}{Converting Boolean Equality to Equivalence}
\begin{description}
\item[syntax:] \texttt{(iff \rest\ \emph{fnums[*]})}
...
\item[\texttt{(iff (4 2 -1))}] : converts the formulas number \texttt{4}, {\tt
2}, and \texttt{-1} into equivalences.
...
\prsubsection{lift-if}{Lifting Embedded IF Connectives}
\begin{description}
\item[syntax:] \texttt{(lift-if \optl\ \emph{fnums} \emph{updates?\default{T}})}
...
branching structure is typically expressed using the \texttt{IF}-connective
or the \indtt{CASES} construct.  Since these \indtt{IF} and \indtt{CASES} branches
could occur embedded within the formula, this branching structure must be
lifted to the top level of the formula where the propositional
simplification steps can be applied (see the \indtt{flatten} and {\tt
split} commands above).    The \indtt{lift-if} rule lifts the
leftmost-innermost contiguous \indtt{IF} or \indtt{CASES} \emph{branching
structure} out to the top level.  An example of such a transformation
is the rewriting of $f(\texttt{IF}(A,B,\texttt{IF}(C,D,E)))$ to ${\tt
IF}(A,f(B),\texttt{IF}(C,f(D),f(E)))$.
On the other hand, $f(\texttt{IF}(\texttt{IF}(A, B, C), D, E))$ is transformed
by \indtt{lift-if} to $\texttt{IF}(A, f(\texttt{IF}(B, D, E)), f(\texttt{IF}(C,
D,E)))$, reflecting the selection of the conditionals of the
leftmost-innermost \texttt{IF}-expression.
Note that $f(\texttt{IF}(A, \texttt{IF}(B, C, D), \texttt{IF}(E, F, G)))$ is
transformed to $\texttt{IF}(A, \texttt{IF}(B, f(C), f(D)), \texttt{IF}(E, f(F),
f(G)))$ reflecting the preservation of the contiguous \texttt{IF}-branching
structure.   It is more effective to lift a
...
previous example are kept independent).  Note that only
conditionals without bound variables can be lifted, and this is used as
a criterion by the \indtt{lift-if} rule in selecting the branching
structure.  The leftmost-innermost branching structure typically turns
out to be the most appropriate one.  If this choice of branching
structure turns out to be inappropriate, the \indtt{case} command (see
below) can be used to carry out the desired case analysis.

\label{if-lift-updates}
Unless the \emph{update?} flag is NIL,  the \indtt{lift-if} command has
been extended to extract the case 
...
number of subgoals requiring identical proofs so {\tt
prop} should be used with some care.  The recursive definition
...
\begin{description}
\item[syntax:] \texttt{(split \optl\ \emph{fnum\default{*} depth})}

\item[effect:] Selects and splits a conjunctive formula in the current
goal sequent based on the information given in \emph{fnum}.  The {\tt
split} command splits every top-level conjunction in the selected
...
\item To get the effect of repeatedly applying  \indtt{flatten} and {\tt
split}, use the \indtt{prop} or \indtt{bddsimp} commands.
\end{itemize}
\end{description}
...
\begin{description}
\item[syntax:] \texttt{(merge-fnums \emph{fnums})}
...
\begin{tabular}{|l|l|p{2.2in}|}\hline
\indttdol{detuple-boundvars} & \emph{defined} & distributes tuple and record
quantification \\\hline
\indttdol{generalize} & \emph{defined} & generalizes term by universal
quantification \\\hline
\indttdol{generalize-skolem-constants} & \emph{defined} & generalizes
Skolem constants\\\hline
\indttdol{inst} & \emph{defined} & instantiates existentially
quantified variables \\\hline
\indtt{instantiate} & \emph{primitive} & instantiates  existentially
quantified variables\\\hline
\indtt{instantiate-one} & \emph{defined} & instantiates existentially
quantified variables\\\hline
\indttdol{inst-cp} & \emph{defined} & copies and instantiates existential formulas \\\hline
\indttdol{inst?} & \emph{defined} & heuristically instantiates existential formulas \\\hline
\indttdol{quant?} & \emph{defined} & heuristically instantiates existential formulas \\\hline
\indtt{skolem} & \emph{primitive} &  \emph{Skolemizes} universal formulas\\\hline
\skobangdol & \emph{defined} & automatically introduces {\em
Skolem} constants \\\hline
\indtt{skolem-typepred} & \emph{primitive} &  \emph{Skolemizes} with
type constraints\\\hline
\indttdol{skosimp} & \emph{defined} & introduces \emph{Skolem} constants and
flattens \\\hline
\indttdol{skosimp*} & \emph{defined} & repeatedly Skolemizes and flattens \\\hline
\end{tabular}

\prdolsubsection{detuple-boundvars}{Distributes Tuple and Record Quantification}
\begin{description}

\item[syntax:] \texttt{(detuple-boundvars \optl\ \emph{fnums}\default{*} \emph{singles?})}

\item[effect:]  A top-level sequent formula of the form
\texttt{(FORALL (x:\ [S1, S2, S3]):\ g(x))} or \texttt{(FORALL (x:\ [\# s : S, t
: T \#]) :\ g(x))} is replaced by
\texttt{(FORALL (x1:\ S1), (x2:\ S2), (x3:\ S3):\ g(x1, x2, x3))} and
\texttt{(FORALL (x1:\ S), (x2:\ T)):\ g((\# s := x1, t := x2 \#)))}\@.
This decomposition of tuple and record quantification is needed, for example, to
carry out an induction over one of the components.  Tuple quantification
can be introduced when instantiating parameterized theories such as the
\texttt{function} theory in the prelude.  This command is used within
the \indtt{measure-induct} strategy.

\end{description}


\prdolsubsection{generalize}{Generalizes Term by Universal Quantification}
\begin{description}

\item[syntax:] \texttt{(generalize \emph{term} \emph{var} \optl\ \emph{type} \emph{fnums}\default{*} \emph{subterms-only?}\default{T})}
...
\prdolsubsection{generalize-skolem-constants}{Generalizes Skolem Constants}
\begin{description}

\item[syntax:] \texttt{(generalize-skolem-constants \optl\  {\em
fnums}\default{*})}
...
\prdolsubsection{inst}{Instantiation of Existential Strength
Quantifiers Without Copying}
\begin{description}

\item[syntax:] \texttt{(inst \emph{fnum} \rest\ \emph{terms})}
...
\prdolsubsection{inst-cp}{Instantiation of Existential Strength
Quantifiers With Copying}
\begin{description}

\item[syntax:] \texttt{(inst-cp \emph{fnum} \rest\ \emph{terms})}
...
\item[syntax:] \texttt{(inst?~\optl\ \emph{fnums\default{*}} \emph{subst}
\emph{where\default{*}} \emph{copy?}~\emph{if-match} \emph{polarity?})}
...
If a partial substitution is given but no match is found and {\em
if-match} is \texttt{nil}, then the rule goes ahead and instantiates
...
that occur free in them.    If the {\em
if-match} flag is \texttt{best}, then the command chooses an
...
sufficient to only give that part of the name preceding \texttt{\char36} in {\em
...
\prsubsection{instantiate}{Instantiating Existential Quantifiers}
\begin{description}
\item[syntax:] \texttt{(instantiate \emph{fnum}
\emph{exprs} \optl\  \emph{copy?})}
...
A)$.  The argument \emph{exprs} provides the list of $n$ terms
...
corresponding $t_i$ in the \emph{exprs} argument.

When the \emph{copy?}\ parameter is \texttt{t}, then a copy of the
quantified formula is saved in the subgoal sequent so that the
quantifier can be reused.  When the \emph{copy?}\ parameter is {\tt
nil}, the quantified formula is automatically hidden so that the
quantifier can be reused by revealing the hidden formulas using the {\tt
reveal} rule.
...
number of bound variables as the length of the supplied list of {\em
exprs}.

\item[{\bf Expecting \ldots\ terms, but \ldots\ terms provided}:]\hfill Wrong
number of terms given in the \emph{exprs} argument.
...
\item[notes:] The defined rule \indtt{inst} is a non-copying form of
\indtt{instantiate}, and the rule \indtt{inst-cp} is a copying form of \indtt{instantiate}.
In the related \indtt{inst?}\ rule, if the \emph{exprs}
argument is missing then the rule attempts to find a suitable
instantiation by matching selected subterms of $A$ with subterms in the
rest of the sequent.  Note that \indtt{inst?}\ does not always succeed
with the right match.  It typically succeeds when there are very few
possible matches.   The \indtt{instantiate} rule captures the $\forall\vdash$
and $\vdash\exists$ sequent rules.

\end{description}

\prsubsection{instantiate-one}{Instantiating Existential Quantifiers}
\begin{description}
\item[syntax:] \texttt{(instantiate-one \emph{fnum} \emph{terms} \optl\ \emph{copy?})}
...
\prdolsubsection{quant?}{Instantiation of Existential Strength Quantifiers}
\begin{description}

\item[syntax:] \texttt{(quant?\ \optl\ \emph{fnum\default{*}} \emph{subst}
\emph{where\default{*}} \emph{copy?}\ \emph{if-match})}

\item[effect:]  This rule is similar to \indtt{inst?}\ and is
retained here for backward compatibility with PVS~1.x.  It should
no longer be used.  

\end{description}

\prsubsection{skolem}{Replacing Universal Quantification with Constants}
\begin{description}
\item[syntax:] \texttt{(skolem \emph{fnum} \emph{constants} \optl\ \emph{skolem-typepreds?})}

\item[effect:] If the formula in the current sequent indicated by {\em
fnum} is either an antecedent formula of the form $(\exists
...
\item[{\bf The types of the skolemized variables contain free occurrences
\ldots}:] This error is
triggered when the top quantifier binds variables $x$ and $y$ where
the type of $y$ contains a free occurrence of $x$.  It is not legal
in this situation to supply a skolem constant for $y$ without
providing one for $x$.  
\end{description}
...
\subsection[\texttt{skolem!}: Automatic Introduction of Skolem Constants]
              {{\hugett skolem!:} Automatic Introduction of Skolem
Constants}\indskobangii

\begin{description}

\item[syntax:] \texttt{(skolem!\@ \optl\ \emph{fnum\default{*}} {\em
keep-underscore?})} 

\item[effect:] Automatical generates Skolem names for the {\em
names} argument for the \indtt{skolem} rule.  These names have the
form \texttt{x!$n$} when the bound variable being named is \texttt{x}.  This
rule is a dangerous one to include in defined rules or within an {\tt
apply} rule since the names being generated could change when a proof is
rerun.  
...
\prdolsubsection{skolem-typepred}{Skolemize with Type Constraints }
\begin{description}

\item[syntax:] \texttt{(skolem-typepred  \optl\ \emph{fnum\default{*}})}
...
\prdolsubsection{skosimp}{Skolemize then Flatten}
\begin{description}

\item[syntax:] \texttt{(skosimp \optl\ \emph{fnum\default{*}} \emph{preds?})}
...
\prdolsubsection{skosimp*}{Repeatedly Skolemize then Flatten}
\begin{description}

\item[syntax:] \texttt{(skosimp* \optl\ \emph{preds?})}

\item[effect:] This is a short form for \texttt{(repeat (then (skolem)
(flatten)))}.  As with \texttt{skosimp}, when the \texttt{preds?} flag is {\tt
T}, the command \texttt{skolem-typepred} is used in place of \texttt{skolem}\@.
\end{description}
...
\begin{tabular}{|l|l|l|}\hline
\indtt{beta} & \emph{primitive} &  reduces $\lambda$-, record, tuple,
update, datatype redexes\\\hline
\indttdol{case-replace} & \emph{defined} & case-split and replace on an
equality\\\hline
\indtt{name} & \emph{primitive} & introduces a name for a complex
expression\\\hline
\indttdol{name-replace} & \emph{defined} & replaces an expression with a
name\\\hline
\indttdol{name-replace*} & \emph{defined} & replaces expressions with names\\\hline
\indtt{replace} & \emph{primitive} & replaces equality LHS for RHS (or
vice-versa)\\\hline
\indtt{replace*} & \emph{primitive} & replaces with equalities\\\hline
\indtt{same-name} & \emph{primitive} & equates two constants with
equal actuals\\\hline 
\end{tabular}

\prsubsection{beta}{Beta Reduction}
\begin{description}
\item[syntax:] \texttt{(beta \optl\ \emph{fnums\default{*}} \emph{rewrite-flag})}
...
it can be shown that $i=j$, and the assignments following {\tt
$(i)$ := $e$} do not affect $f(j)$.  If it can be shown using the
...
\prdolsubsection{case-replace}{Introducing and Applying Equality}
\begin{description}
\item[syntax:] \texttt{(case-replace \emph{expr})}

\item[effect:] This is just the strategy
\texttt{(then@ (case \emph{expr})(replace -1))}.  It case splits on an
equality (note that a non-equality $A$ is interpreted as an equality
$A = \mbox{\sc true}$) and replaces the LHS by the RHS in the rest
of the sequent.  

\item[usage:]  \texttt{(case-replace "c = 0")}: Replaces \texttt{c} by \texttt{0}
in the current subgoal and generate a second subgoal with the
...
\prsubsection{name}{Introducing Names for Terms}
\begin{description}
\item[syntax:] \texttt{(name \emph{name} \emph{expr})}
...
\prdolsubsection{name-replace}{Replacing a Term by a Name}
\begin{description}
\item[syntax:] \texttt{(name-replace \emph{name} \emph{expr} \optl\ \emph{hide?\default{T}})}
...
\prdolsubsection{name-replace*}{Replacing  Terms by Names}
\begin{description}
\item[syntax:] \texttt{(name-replace* \emph{name-and-exprs} \optl\ \emph{hide?\default{T}})}
...
\prsubsection{replace}{Rewriting Using Equalities}
\begin{description}
\item[syntax:] \texttt{(replace \emph{fnum} \optl\
\emph{fnums\default{*}} \emph{dir\default{LR} hide?\ actuals?})}
...
listed in the \emph{fnums} argument.  When the \emph{direction}
argument is \texttt{RL} (denoting ``right-to-left''), the target
occurrences of $r$ in the sequent are rewritten to $l$.  Otherwise, when
the \emph{direction} parameter is different from \texttt{RL}, the target
occurrences of $l$ are rewritten to $r$.  If \emph{fnum} is the number
for an antecedent formula $A$ that is not an equality, then the formula
is regarded as an equality of the form $A = \mbox{\sc true}$.  If {\em
fnum} is the number of a consequent formula $A$, then the formula is
regarded by \indtt{replace} as an equality of the form $A = \mbox{\sc
false}$.  Note that the formula indicated by \emph{fnum} is unaffected
by \indtt{replace}.
...
actual parameters of names.   Otherwise, the replacement only occurs
...
\prsubsection{replace*}{Rewriting Using Equalities}
\begin{description}
\item[syntax:] \texttt{(replace* \rest\ \emph{fnums})}
...
\prsubsection{same-name}{Equating Names Under Actuals}
\begin{description}
\item[syntax:] \texttt{(same-name \emph{name1} \emph{name2} \optl\ \emph{type})}

\item[effect:] Two names such as $\texttt{cons[\{i : nat | i > 0\}]}$
and $\texttt{cons[\{i : nat | i /= 0\}]}$ can be denotationally equal
yet syntactically distinct.  The command \indtt{same-name} can be used
to introduce an antecedent formula asserting the equality of two such
names, \eg, $\texttt{cons[\{i : nat | i > 0\}]} = \texttt{cons[\{i : nat | i /=
0\}]}$, 
while generating the proof obligations required to show that
the actuals coincide.  In the above case, the obligation would be
to show that \texttt{FORALL (i:\ nat):\ i > 0 IFF i /= 0}\@.
...
\prsubsection{expand}{Expanding Definitions}
\begin{description}
\item[syntax: \texttt{(expand \emph{name \optl\ fnum\default{*} occurrence if-simplifies assert?})}]

\item[effect:] Expands (and simplifies) the definition of \emph{name} at
a given \emph{occurrence}.  If \emph{occurrence} is not given, then all
...
counting from the left, or as a list of such numbers.  If the {\em
if-simplifies} flag is \texttt{t}, then any expansion within a sequent
formula occurs only if the expanded form can be simplified (using the
decision procedures).  The \emph{if-simplifies} flag is needed to control
infinite expansions in case \indtt{expand} is used repeatedly inside a
strategy.  In the default case when \emph{assert?}\ is \texttt{NIL},
\indtt{expand} applies the \indtt{simplify} step with the default settings
to any sequent formula in which a definition is expanded.  When {\em
assert?}\ is \texttt{T}, \indtt{expand} applies the \indtt{assert} version
of \indtt{simplify} to any sequent formulas affected by definition
expansion (This latter option has to be exercised for compatibility with
PVS~1.x).  The \emph{assert?}\ flag can also be \texttt{NONE} in which case
no simplification is applied to the sequent formula following expansion.
...
\item[\texttt{(expand "sum" 1 2)}] : Expands the second occurrence of {\tt
sum} in the formula labeled \texttt{1}.
...
\prdolsubsection{expand*}{Expanding Several Definitions}
\begin{description}
\item[syntax: \texttt{(expand* \rest\ \emph{names})}]
...
\prdolsubsection{forward-chain}{Forward Chaining}
\begin{description}
  \item[syntax:] \texttt{(forward-chain \emph{name-or-fnum})}
  \item[effect:] This rule is used to forward chain on the given
lemma or antecedent formula number.  If the given lemma or antecedent
  formula has the form $A_1   \wedge \ldots\wedge A_n 
\supset C$, then this rule tries to match the formulas $A_i$
against the antecedent formulas of the current sequent.  If the
match succeeds, the corresponding instance of $C$ is added to the
as an antecedent formula to current sequent.  If this instance of $C$
is already in the current sequent, then the \indtt{forward-chain} rule
looks for other instances of the $A_i$ in the current sequent.
An \emph{fnum} argument can be either \texttt{-}, \texttt{*}, or
an antecedent formula number. 


\end{description}

\prsubsection{lemma}{Employing Instances of Lemmas}
\begin{description}
\item[syntax:] \texttt{(lemma \emph{name} \optl\ \emph{subst})}

\item[effect:] This rule introduces an instance of the lemma named {\em
name} corresponding to the substitutions supplied in \emph{subst} as
...
\end{enumerate}
In such a situation, the \indtt{lemma} rule picks the first form if {\em
subst} contains substitutions for $x, y, u$, and $v$, and in general,
...
t_n)$.  Let $A$ be the universally closed form of the lemma named {\em
name}, the lemma rule checks that each $x_i$ in \emph{subst} is a
...
be supplied in a more complete form.  The form {\em
identifier[actuals]} for \emph{name} can be used in the case when the
...
\prdolsubsection{rewrite}{Matching and Rewriting with Lemmas}
\begin{description}

\item[syntax:] \texttt{(rewrite \emph{name} \optl\ {\em
fnums\default{*}} \emph{subst} \emph{target-fnums\default{*}} {\em
dir\default{LR}} \emph{order\default{IN}})}
...
the conclusion of the lemma against expressions in the formulas in {\em
fnums}.  This rule is always to be preferred to \indtt{rewrite-lemma}
since it does the hard work of figuring out the substitutions.  The {\em
target-fnums} corresponds to the \emph{fnums} argument of {\tt
rewrite-lemma}.  If the \emph{order} argument is \texttt{IN} (which is the
default), then the matching is to be done inside-out, and if it is {\tt
OUT}, the matching is done outside in.
...
\prdolsubsection{rewrite-lemma}{Rewriting Using a Lemma}
\begin{description}
\item[syntax:] \texttt{(rewrite-lemma \emph{lemma} \emph{subst}
\optl\ \emph{fnums\default{*}} \emph{dir\default{LR}})}
...
and each $x_i$ is the identifier for a substitutable variable in {\em
name}, \ie\ one that is either a free variable or is universally
...
\prdolsubsection{rewrite-with-fnum}{Matching and Rewriting with
Antecedent Formulas}
\begin{description}

\item[syntax:] \texttt{(rewrite-with-fnum \emph{fnum} \optl\ \emph{subst} \emph{fnums\default{*}} \emph{dir\default{LR}})}

\item[effect:] Applies the \texttt{rewrite} command to an antecedent formula
indicated by \emph{fnum} which is used as a rewrite rule.  The input
substitution \emph{subst} is used to guide the matching process to find a
match that extends \emph{subst}\@.  The optional \emph{fnums} argument
is used to direct the command to look for matches within the sequent
formulas indicated by \emph{fnums}\@.  When \emph{dir} is \texttt{LR}, the
antecedent formula is used a rewrite rule in the left-to-right direction,
and when it is \texttt{RL}, the rewriting occurs in the right-to-left
direction.  
\end{description}

\prdolsubsection{use}{Invoke lemma with heuristic instantiation and beta-reduction}
\begin{description}

\item[syntax:] \texttt{(use \emph{lemma} \optl\ \emph{subst}
\emph{if-match\default{\texttt{best}}})}

\item[effect:]  An extension of the \indtt{lemma} command where the
formula introduced is subject to repeated 
instantiation and beta-reduction using the \indtt{inst?}\ and \indtt{beta}
commands.  This is usually an effective alternative to the \indtt{lemma}
command.    The \emph{subst} argument is as in the \indtt{lemma}
command.  The \emph{if-match} command is as in the \texttt{inst?}\ command.
The possible values for if-\texttt{match} are:
\begin{itemize}
\item \texttt{all}: Find all instances of the first matching template in the formula
being instantiated.
\item \texttt{best}: Find the best instance (i.e., one that generates the fewest
TCCs) for the first matching template.

\item \texttt{T}: Ignore the partial substitution given unless some matching
template was found.  
...
\prdolsubsection{use*}{Invoke lemmas with heuristic instantiation and beta-reduction}
\begin{description}

\item[syntax:] \texttt{(use* \rest\ \emph{names})}
...
\indttdol{apply-extensionality} & \emph{defined} & uses extensionality to
prove equality \\\hline
\indttdol{apply-eta} & \emph{defined} & uses eta form extensionality
\\\hline
\indttdol{decompose-equality} & \emph{defined} & Reduce equality to
component equalities \\\hline
\indttdol{eta} & \emph{defined} & introduces eta version of extensionality\\\hline
\indtt{extensionality} & \emph{primitive} & introduces extensionality
axiom \\\hline
 \indttdol{replace-eta} & \emph{defined} & replace using eta\\\hline
\indttdol{replace-extensionality} & \emph{defined} & replace using
extensionality \\\hline
\end{tabular}
...
\prdolsubsection{apply-extensionality}{Apply Extensionality to
an Equality}
\begin{description}

\item[syntax:] \texttt{(apply-extensionality \optl\ \emph{fnum\default{+}}
\emph{keep?}\ \emph{hide?})}
...
type of $l$ and $r$ has a corresponding extensionality axiom scheme, {\tt
apply-extensionality} will generate a new succedent that is the result
of using \indtt{replace-extensionality} on $l$ and $r$.

If the \emph{keep?}\ flag is set to \texttt{T}, the antecedent equality
introduced by the \indtt{apply-extensionality} command is retained
in the resulting goal sequent.
...
\item[syntax:] \texttt{(apply-eta \emph{term} \optl\ \emph{type})}
...
\prdolsubsection{decompose-equality}{Decomposes Equality to Component
Equalities}
\begin{description}
\item[syntax:] \texttt{(decompose-equality \optl\ \emph{fnum}\default{*} {\em
hide?}\default{T})}
...
\indtt{decompose-equality} rule is the same as {\tt
apply-extensionality}\@.    
\end{description}

\prdolsubsection{eta}{An Axiom Scheme for Functions,
                         Records, Tuples, and Abstract Datatypes}
\begin{description}
\item[syntax:] \texttt{(eta \emph{type})}
...
\prsubsection{extensionality}{An Axiom Scheme for Functions,
                         Records, Tuples, and Abstract Datatypes}
\begin{description}
\item[syntax:] \texttt{(extensionality \emph{type})}

\item[effect:] The \indtt{extensionality} rule is similar to the {\tt
lemma} rule in that it introduces an extensionality axiom for the given
...
\begin{description}
\item[{\bf The following irrelevant free variables occur in the given
type ex-}]{\bf pression}: As with the other rules, no free variables can be
introduced into a proof sequent through a rule application.
...
\item[syntax:] \texttt{(replace-eta \emph{term} \optl\ \emph{type} {\em
keep?})}
...
\item[syntax:] \texttt{(replace-extensionality \emph{expr1} \emph{expr2}
\optl\ \emph{expected} \emph{keep?})}

\item[effect:] This rule is an extension of the \indtt{extensionality}
rule.  It uses the relevant extensionality axiom scheme to demonstrate
the equality of \emph{expr1} and \emph{expr2} and replaces {\em
expr1} in the sequent with \emph{expr2}.  In some cases, the optional
...
\indttdol{induct} & \emph{defined} & invokes induction\\\hline
\indttdol{induct-and-rewrite} & \emph{defined} & invokes induction and
simplifies\\\hline
\indbang{} & {\em
defined} & induct and simplify with definitions\\\hline
\indttdol{induct-and-simplify} & \emph{defined} & induction followed by
simplification \\\hline
\indttdol{measure-induct} & \emph{defined} & invokes measure
induction\\\hline
\indttdol{measure-induct+} & \emph{defined} & a better {\tt
measure-induct}\\\hline
\indttdol{measure-induct-and-simplify} & \emph{defined} & measure
induction with simplification\\\hline
\indttdol{name-induct-and-rewrite} & \emph{defined} & invoke induction
scheme by name\\\hline
\indttdol{rule-induct} & \emph{defined} & induction on inductive
relation\\\hline
\indttdol{rule-induct-step} & \emph{defined} & induction on inductive
relation\\\hline
\indttdol{simple-induct} & \emph{defined} & introduces instance of
induction scheme\\\hline
\indttdol{simple-measure-induct} & \emph{defined} & introduces instance of
induction scheme\\\hline
\end{tabular}
\prdolsubsection{induct}{Automatically Employ Induction Scheme}
\begin{description}

\item[syntax:] \texttt{(induct \emph{var} \optl\ {\em
fnum\default{1}} \emph{name})}

\item[effect:] The formula indicated by \emph{fnum} must be a
universally quantified, consequent formula.  The variable name {\em
var} must be quantified at the outermost level of this formula.
...
\item[errors:]
\begin{description}
\item[{\bf Could not find suitable induction scheme}] : The {\tt
simple-induct} rule invoked by this step failed to find an induction
scheme for the given variable \emph{var}, \emph{fnum}, and {\em
name}\@.  Check that \emph{var} occurs as an outermost universally
...
\prdolsubsection{induct-and-rewrite}{Induct then Rewrite/Simplify}
\begin{description}

\item[syntax:] \texttt{(induct-and-rewrite  \emph{var} \optl\ {\em
fnum\default{1}} \rest\ \emph{rewrites})}

\item[effect:] This command has been superseded in PVS~2 by the more general
\indtt{induct-and-simplify} but is retained for backward
compatibility with PVS~1\@.  It employs \indtt{induct} on \emph{variable}
and \emph{fnum} to select, instantiate, and introduce an induction
scheme, and then uses the rewrite rules given in \emph{rewrites} to
simplify the resulting base and induction cases employing
\indtt{skosimp*}, \indtt{assert}, \indtt{lift-if}, \indtt{inst?}, and
\indtt{bddsimp}\@.

\begin{usage}{}
\item[\texttt{(induct-and-rewrite "x" 1 "append" "reverse")}] :
Introduces an instance of the induction scheme according to variable {\tt
x} obtained by instantiating it with the predicate formed from formula
...
\prdolsubsection{induct-and-simplify}{Induct then Rewrite/Simplify}
\begin{description}
\item[syntax:] \texttt{(induct-and-simplify  \emph{var} \optl\ {\em
fnum\default{1}} \emph{name} \emph{defs\default{\texttt{T}}}
\emph{if-match\default{\texttt{best}}} \emph{theories} \emph{rewrites}
\emph{exclude})}
...
\indtt{install-rewrites} to install the rewrites in \emph{defs},  {\em
theories} and \emph{rewrites} while excluding those in \emph{exclude},
invokes the the \indtt{induct} command on \emph{var}, \emph{fnum}
and \emph{name} to get the base and induction cases, which are
then simplified by repeated application of \indtt{skosimp*},
\indtt{assert}, \indtt{lift-if}, \indtt{bddsimp}, and \indtt{inst?}\ 
(as controlled by the \emph{if-match} argument)\@.
...
explicit definitions), the rewrites in the  prelude theory {\tt
real\_props} but excluding 
...
\prdolsubsection{measure-induct}{Invoke Measure Induction}
\begin{description}
\item[syntax:] \texttt{(measure-induct  \emph{measure} \emph{vars} \optl\
\emph{fnum\default{1} order skolem-typepreds?})}

\item[effect:] The command \texttt{measure-induct+} is the preferred way to
invoke measure induction.  The \texttt{measure-induct} command takes a {\em
measure} expression and a list of the \emph{induction} variables {\em
vars} in which the measure is defined.  These variables must occur
universally quantified in the formula numbered \emph{fnum}.  This list of
variables is needed in order to unambiguously identify those universally
quantified variables on which the measure is defined.  As with
\indtt{induct}, the \texttt{measure-induct} command forms the induction
predicate by lambda-abstracting the formula over the variables given in
\emph{vars}.  The measure function is also obtained by lambda-abstracting
the given \emph{measure} over the variables in \emph{vars}\@.  The {\tt
measure\_induction}\index{measure\_induction@{\texttt{measure\_induction}}}
...
well-founded ordering is taken by default to be \texttt{<} on natural numbers
or ordinals unless a different ordering is given through the \emph{order}
argument.  The \indtt{lemma} rule is used to introduce the measure
induction scheme instantiated with the selected induction predicate.  The
work so far is actually carried out by the \indtt{simple-measure-induct}
proof step.  The \indtt{measure-induct} step then beta-reduces,
simplifies, and instantiates the measure induction lemma to discharge the
goal sequent and in the process generates an induction subgoal consisting
of an antecedent induction hypothesis and a consequent induction
conclusion.  If \emph{skolem-typepreds?} is T, then typepreds are
introduced for any introduced skolem constants.

The problem with {\tt
measure-induct} is that the arrangement of quantifiers in the induction
hypothesis is unhelpful.  If the formula numbered \emph{fnum} has the
form \texttt{(FORALL x, w:\ p(x, w))} where the induction variable is \texttt{x}
and the measure is \texttt{m}, the induction predicate is
\texttt{(LAMBDA x:\ (FORALL w:\ p(x, w)))}, and the 
resulting induction hypothesis has the form
\texttt{(FORALL x:\ m(x) < m(c) IMPLIES (FORALL w:\ p(x, w)))}.  This form
nests the universal quantification on \texttt{w} which might be useful in
guiding the instantiation of \texttt{x}\@.  Therefore a 
more useful form of the induction hypothesis is with quantification
rearranged as 
\texttt{(FORALL x, w:\ m(x) < m(c) IMPLIES p(x, w))}\@.  This rearrangement
is carried out by \indtt{measure-induct+}\@.  See \indtt{measure-induct+}
for usage.
...
\begin{description}
\item[syntax:] \texttt{(measure-induct+  \emph{measure} \emph{vars} \optl\
\emph{fnum\default{1}} \emph{order})}
...
T, then typepreds are introduced for any introduced skolem constants.

The problem with 
\indtt{measure-induct} is that the arrangement of quantifiers in the induction
hypothesis is unhelpful.  If the formula numbered \emph{fnum} has the
form \texttt{(FORALL x, w:\ p(x, w))} where the induction variable is \texttt{x}
and the measure is \texttt{m}, the induction predicate is
\texttt{(LAMBDA x:\ (FORALL w:\ p(x, w)))}, and the 
...
\texttt{(FORALL x:\ m(x) < m(c) IMPLIES (FORALL w:\ p(x, w)))}.  This form
nests the universal quantification on \texttt{w} which might be useful in
guiding the instantiation of \texttt{x}\@.   A 
more useful form of the induction hypothesis is with quantification
rearranged as 
\texttt{(FORALL x, w:\ m(x) < m(c) IMPLIES p(x, w))}\@.  This rearrangement
is carried out by \indtt{measure-induct+}\@.  The command
...
\prdolsubsection{measure-induct-and-simplify}{Measure Induction
followed by Simplification and Rewriting}
\begin{description}
\item[syntax:] \texttt{(measure-induct-and-simplify \emph{measure} {\em
vars} \optl\ \emph{fnum\default{1}} \emph{order} \emph{expand} {\em
defs\default{T}}
\emph{if-match\default{best}} \emph{theories} \emph{rewrites} \emph{exclude})}

\item[effect:]  Invokes the appropriate instance of measure induction
using \indtt{measure-induct+}, skolemizes the resulting induction
conclusion, and expands the definitions listed in the \emph{expand} argument
to then generate the corresponding cases.  The resulting subgoals
are then simplified and the induction hypothesis is instantiated
by heuristic matching and each subgoal is subject to 
further propositional splitting and simplification based on rewriting and
decision procedures.  This command is very similar to
\indtt{induct-and-simplify} but employs measure induction and
uses the \emph{expand} argument to guide the case analysis.
If multiple instances of the induction hypothesis are needed, the
\emph{if-match} argument can be given as \texttt{first*} to obtain
all instances of the first matching template in the quantified formula,
or \texttt{all} to obtain all matches for all templates.

\begin{usage}{}
\item[]\texttt{(measure-induct-and-simplify "length(x) + length(y)" ("x" "y")
:expand "merge" :theories "merge\_sort")}:
...
Since there are multiple recursive calls in the recursive case of {\tt
quicksort}, the \texttt{first*} option to \emph{if-match} is used.
\end{usage}

\end{description}

\prdolsubsection{name-induct-and-rewrite}{Induction on a named scheme
followed by simplification and rewriting}
\begin{description}
\item[syntax:] \texttt{(name-induct-and-rewrite  \emph{var} \optl\ {\em
fnum\default{1}} \emph{name} \rest\ \emph{rewrites})}
...
\prdolsubsection{rule-induct}{Induction on an inductive relation}
\begin{description}
\item[syntax:] \texttt{(rule-induct  \emph{rel} \optl\ {\em
fnum\default{+}} \emph{name})}

\item[effect:]  Applies the induction scheme given by an inductive
relation \emph{rel} to a sequent of the form:
$$ \ldots \vdash (\forall x_1, \ldots, x_n: {\it rel}(x_1, \ldots, x_n)
\supset \ldots)$$
or, alternately, the form:
$$ \ldots, {\it rel}(c_1, \ldots, c_n), \ldots \vdash \ldots$$
The induction schemes corresponding to an inductive relation \emph{rel}
are \texttt{\emph{rel}\_weak\_induction} and \texttt{{\em
rel}\_induction}\@.  

This command  applies repeated skolemization  and flattening
to the specified \emph{fnum} (or the first positive, skolemizable consequent)
before invoking \indtt{rule-induct-step} on the resulting subgoal.   The
strategy uses the weak induction scheme by default but can be told to use
strong induction by supplying \texttt{\emph{rel}\_induction} as the
\emph{name} argument. 

\end{description}

\prdolsubsection{rule-induct-step}{Induction on an inductive relation}
\begin{description}
\item[syntax:] \texttt{(rule-induct-step  \emph{rel} \optl\ {\em
fnum\default{-}} \emph{name})}
...
supplying \texttt{\emph{rel}\_induction} as the \emph{name}
argument.

\end{description}

\prdolsubsection{simple-induct}{Introduce induction scheme instance}
\begin{description}
\item[syntax:] \texttt{(simple-induct  \emph{var} \emph{fmla} \optl\ {\em
name})}
...
\prdolsubsection{simple-measure-induct}{Introduce induction scheme instance}
\begin{description}
\item[syntax:] \texttt{(simple-measure-induct measure vars \optl\  \emph{fnum\default{1}} \emph{order})}
...
Inserts measure induction on the measure \texttt{i + j} in the variables {\tt
i} and \texttt{j}\@.  
...
\begin{tabular}{|l|l|p{2.75in}|}\hline
\indttdol{assert} & \emph{defined} & uses decision procedures to assert
sequent formulas \\\hline
\indttdol{bash} & \emph{defined} & \texttt{assert}, \texttt{bddsimp}, \texttt{inst?},
\texttt{skolem-typepred}, \texttt{flatten}, \texttt{lift-if} \\\hline
\indttdol{both-sides} & \emph{defined} & Applies operation to both sides
of inequality chain \\\hline
\indttdol{do-rewrite} & \emph{defined} & applies auto-rewrites\\\hline
\indttdol{grind} & \emph{defined} & installs rewrites and repeatedly
simplifies \\\hline
\indttdol{ground} & \emph{defined} & propositional and ground
simplification \\\hline
% \indtt{ground-forward-chain} & \emph{primitive} %
% &declares a list of formulas as ground forward chain rules\\\hline
% \indtt{ground-forward-chain-theories} & \emph{defined} &  declares
% theories to be used as ground forward chain rules\\\hline
\indttdol{record} & \emph{defined} & records assumptions for the decision
procedures\\\hline
\indttdol{reduce} & \emph{defined} & Repeated simplification\\\hline
\indtt{simplify} & \emph{primitive} & does Boolean simplification using
decision procedures\\\hline
\indttdol{simplify-with-rewrites} & \emph{defined} & installs rewrites,
simplifies, removes rewrites\\\hline
\indttdol{smash} & \emph{defined} & \indtt{ground} with lift-if\\\hline
\end{tabular}

Version 2.3 features a new implementation of the ground decision
procedures.  The new implementation overcomes several deficiencies
of the old ground decision procedures originally implemented by
Rob Shostak.   The original decision procedures are also
included in Version~2.3 for the purposes of backward compatibility which
might be necessary in some rare cases.  For the most part, the new
decision procedures should subsume the old ones. 
The PVS Emacs command \emacstt{new-decision-procedures} switches from the
old to the new decision procedures and \emacstt{old-decision-procedures}
can be used to switch back to the old ones.  \comment{The commands
\indtt{ground-forward-chain}, \indtt{ground-rewrite}, and
\indtt{install-ground-rewrites} are relevant only when the new decision
procedures are being employed.  }



\prsubsection{assert}{Simplifying Using the Decision Procedures}

\begin{description}

\item[syntax:] \texttt{(assert \optl\ \emph{fnums\default{*}} {\em
rewrite-flag} \emph{flush?}\ \emph{linear?} \emph{cases-rewrite?}\ {\em
type-constraints?\default{T}} \emph{ignore-prover-output?})}

\item[effect:]
The \indtt{assert} rule is a combination of \indtt{record}, \indtt{simplify},
\indtt{beta}, and \indtt{do-rewrite}.  
The use of decision procedures for equalities and linear
inequalities is perhaps the most significant and pervasive part of
\pvs\@.  These procedures are invoked to prove trivial theorems, to
simplify complex expressions (particularly definitions), and even to
perform matching.  These decision procedures, originally due to Shostak,
employ congruence closure for equality reasoning, and they also perform
linear arithmetic reasoning over the natural numbers and reals.  They
deal solely with \emph{ground} formulas, namely those that contain no
quantifiers.  While they primarily deal with linear arithmetic, \ie\
expressions of the form \texttt{2*x + 3*y <= 4*z}, there are some modest
extensions for dealing with expressions involving nonlinear subterms
using simplifications such as \texttt{(x + y)*(x - y) = (x*x - y*y)} and
simplifications involving division such as \texttt{x*(y/x) = y}.
...
only those formulas that do not contain branching (\texttt{if} or {\tt
cases}) or propositional structure (unless they are within a
...
constraints on subexpressions of the formulas processed by {\tt
assert} are also automatically recorded by the decision procedures for
the commands \indtt{assert}, \indtt{record}, \indtt{simplify}, and {\tt
do-rewrite}.  The new assertions in the data-structures remain valid
...
rewrite expressions using the rewrite rules provided by the {\tt
auto-rewrite} and \indtt{auto-rewrite-theory} rules below (see {\tt
do-rewrite}).  In order for any automatic rewrites to take effect, it
...
any equality formula is simplified since simplifying the left-hand side
of a formula to be used by \indtt{replace} in the process of rewriting
(see strategies \indtt{rewrite} and \indtt{rewrite-lemma} below) could cause
...
used by the decision procedures is flushed.  This database can get
fairly large in the course of a proof thereby decreasing the efficiency
of the assert rule.  The \emph{flush?}\ flag should be used with
caution.  The decision procedures apply a modest amount of non-linear
arithmetic simplification  to multiplication and division
expressions.  This can sometimes get in the way.  the {\em
linear?}\ argument can be set to \texttt{t} in order to prevent such
simplifications.  
...
The ground prover when confronted with a non-convex assertion returns
a disjunction of assertions that are equivalent to the input assertion.
The simplification commands examine these outputs to see if every path
through them yields a contradiction.   This step can in some cases be
expensive.  The \texttt{ignore-prover-output?} flag can be set to NIL in
order to avoid the examination of the ground prover outputs. 
...
that rely on these constraints can be missed in the first pass of {\tt
assert}.  

Another point about the \indtt{assert} rule is that is that from the
point of view of the control mechanism used for strategies, {\tt
assert} almost always succeeds so that a strategy like \texttt{(repeat
(assert))} is certain to get into an infinite loop.  The typical way
to get around this is to have \indtt{assert} follow some other step {\em
step1} that is guaranteed to not repeat indefinitely, as in:
\begin{alltt}
...
\prdolsubsection{bash}{Assert, bddsimp, inst?,
skolem-typepred, flatten, and lift-if }

\begin{description}
\item[syntax:]  \texttt{(bash \optl\ \emph{if-match}\default{T} {\em
updates?}\default{T} \emph{polarity?})}

\item[effect:] This command executes \indtt{assert}, \indtt{bddsimp},
\indtt{inst?}, \indtt{skolem-typepred}, \indtt{flatten}, and
\indtt{lift-if}, in that order.  This command is the core of the
\indtt{reduce} command which in turn is the workhorse of \indtt{grind}\@.

As in the \indtt{inst?}\ command,  the \emph{if-match} option
can be \texttt{NIL}, \texttt{T}, \texttt{ALL}, or \texttt{BEST} for no, some, all, or
the best instantiation, respectively.   Note that \indtt{inst?}\  
precedes \indtt{skolem-typepred}, so that matches that are in the
original sequent are preferred to those that are exposed by
Skolemization.  This has the drawback that quite often, it is better
to instantiate following Skolemization.  To avoid eager instantiation,
\indtt{bash} should be invoked with \emph{if-match} set to \texttt{NIL}\@.
...
\prdolsubsection{both-sides}{Applies Operation to Both Sides
of Inequality Chain}

\begin{description}
\item[syntax:] \texttt{(both-sides \emph{op} \emph{term} \optl\ {\em
fnum}\default{1})}

\item[effect:] Here \emph{fnum} is used to find a chained conjunction of
inequalities of the form \texttt{e1 <= e2 AND e2 <= e3 AND e3 <= e4}\@.
If the \emph{term} argument is \texttt{t}, 
the command replaces the above chain with
\texttt{e1 op t <= e2 op t AND e2 op t <= e3 op t AND e3 op t <= e4 op t}\@.
If the equivalence between this chain and the previous one doesn't
simplify to \texttt{TRUE} using \indtt{assert} and \indtt{do-rewrite} with
respect to the prelude theory \texttt{real\_props}, then a proof obligation is
generated.
\end{description}

\prdolsubsection{do-rewrite}{Applying auto-rewrites}

\begin{description}
\item[syntax:] \texttt{(do-rewrite \optl\ \emph{fnums\default{*}}
  \emph{rewrite-flag} \emph{flush?}\ \emph{linear?} {\em
  cases-rewrite?}\ \emph{type-constraints?\default{T}})}
...
\prdolsubsection{grind}{Install Rewrites and Repeatedly Simplify}
\begin{description}
\item[syntax:] \texttt{(grind \optl\ \emph{defs}\default{!} \emph{theories}
\emph{rewrites} \emph{exclude} \emph{if-match}\default{T} {\em
updates?}\default{T} \emph{polarity?})}

\item[effect:]  This is a catch-all strategy that is frequently used to
automatically complete a proof branch or to apply all the 
obvious simplifications till they no longer apply.  The strategy
first applies \indtt{install-rewrites} to install the given theories and
rewrite rules along with all the relevant definitions in the given
subgoal.  It then applies \indtt{bddsimp} followed by \indtt{assert}
(similar to \indtt{ground})
to carry out the first level of simplification.
This is followed by \indtt{replace*} to carry out all the
equality replacements.  This is followed by \indtt{reduce} which
repeatedly applies \indtt{bash} (which invokes \indtt{assert}, \indtt{bddsimp},
\indtt{inst?}, \indtt{skolem-typepred}, \indtt{flatten}, and
...
The \emph{if-match} and \emph{polarity?}\ are used to control
heuristic instantiation using \indtt{inst?}\@.  Note that by
setting \emph{if-match} to \texttt{NIL}, one can avoid the eager
instantiation behavior of \indtt{grind}\@.  A second \indtt{grind} can
then be used to pick up the instantiations exposed by the first
instantiation-free \indtt{grind}\@.
...
\begin{description}
\item[syntax:] \texttt{(ground)}

\item[effect:]   This command invokes propositional simplification followed
by \texttt{assert}\@.   It is useful in obtaining 
simplified forms of the cases arising from propositional simplification.
These simplifications include those given by \indtt{assert}, namely,
the various boolean, datatype, and arithmetic simplifications,
beta-reduction,  simplification using ground decision procedures, and
rewriting with respect to the installed rewrite rules.

\end{description}

\comment{
\prsubsection{ground forward chain}{Installing Automatic Ground Forward
Chain Rules}
\begin{description}

\item[syntax:] \texttt{(ground-forward-chain \rest\ \emph{names})}
...
the current proof node as automatic ground forward chain rules to be used
for simplification by the \texttt{assert} rule.  Only formulas of a certain
special form can be considered as automatic ground forward chain rules.
An automatic ground forward chain rule is of the form $A_1 \wedge
\ldots\wedge A_n \supset C$.

An example forward chain formula is: $x >= y \wedge y >= x \supset x = y$.
After this rule has been declared a ground forward chain rule whenever the
decision procedures discover in its state for some ground terms $t_1,
t_2$, that $t_1 >= t_2$ and $t_2 >= t_1$, it will automatically add $t_1 =
t_2$ to its set of known facts.  It will then be able to automatically
discharge sequents that previously would require the user to explicitly
invoke the AntiSymmetry axiom.

It is important to note that \indtt{ground-forward-chain} has no visible effect
on the sequent, but  
it affects the subsequent behavior of \indtt{assert} in any lower branch
of the proof.  The scope of an \indtt{ground-forward-chain} declaration is
restricted to the branch of the proof below it.

The \indtt{ground-forward-chain} command works only in conjunction with
the new decision procedures.   


\item[usage:] \texttt{(ground-forward-chain "AntiSymmetry" "Transitivity")}

\item[errors:] Due to the presence of actual parameters, {\tt
ground-forward-chain} can generate parsing and typechecking errors, in
addition to those listed below.
\begin{description}

\item[{\bf No resolutions for \ldots}:] The system was unable to find
ground forward chain declarations corresponding to the given names.
\end{description}

\end{description}

\prdolsubsection{ground-forward-chain-theory}{Automatic Ground Forward 
Chaining Relative to a Theory}

\begin{description}

\item[syntax:] \texttt{(ground-forward-chain-theory \emph{name} \optl\  {\em
exclude} \emph{tccs?})}

\item[effect:] 	 "Takes all the ground forward chain rules in the theory
\emph{name} and declares these to be ground forward chain rules.
Declarations named in EXCLUDE are not introduced.
By default, TCCs in the theory are excluded but they can be included
when the TCC? flag is T.  When \texttt{ground-forward-chain-theory} is
applied to the current theory, all of the ground forward chain rules that
precede the formula being proved in the theory are declared to be
automatic.

The \indtt{ground-forward-chain-theory} command works only in conjunction with
the new decision procedures.   


\begin{usage}{}

\item[]\texttt{(ground-forward-chain-theory "ban_second_logic")}
\end{usage}
...
appear in the current context.  Add the theory to the \texttt{USING} list
if needed.
...
\prdolsubsection{record}{Recording assumptions for the decision procedures}
\begin{description}
\item[syntax:] \texttt{(record \optl\ \emph{fnums\default{*}}
  \emph{rewrite-flag} \emph{flush?}\ \emph{linear?}\ {\em
  cases-rewrite?} \emph{type-constraints?\default{T}}
  \emph{ignore-prover-output?})}

\item[effect:]  The decision procedures maintain efficient
data structures where the assumptions that are true in the current
context are recorded.  The record command is used to add more
assumptions to these data structures.  The only assumptions that can be
recorded are those that do not contain any embedded \texttt{if}, {\tt
cases}, or boolean structure.  The assumptions are antecedent formulas
and negations of consequent formulas.  It is possible for a \indtt{record}
command to prove the sequent if the assumptions are found to be
contradictory.  The other arguments are
as in \indtt{assert}\@.
...
\item[notes:]  a formula is simplified before it is recorded (see {\tt
simplify} and \indtt{assert} below) so that it is possible for {\tt
record} to record an
assumption to contain \texttt{if}, \texttt{cases} or boolean structure that is
eliminated by simplification.   the command \indtt{assert} subsumes {\tt
record} but its behavior is more difficult to explain.

\end{description}

\prdolsubsection{reduce}{Repeated Simplification}
\begin{description}
\item[syntax:] \texttt{(reduce \optl\ \emph{if-match}\default{T} {\em
updates?}\default{T} \emph{polarity?})}

\item[effect:]  This command is the main workhorse of the \indtt{grind}
command.  It applies \indtt{bash} followed by \indtt{replace*}
in a loop until neither command has any effect.

The \emph{if-match} and \emph{polarity?}\ options are as in
\indtt{bash} and control the instantiation.
...
\begin{description}
\item[syntax:] \texttt{(smash \optl\ \emph{updates?}\default{T})}

\item[effect:]   This command is a more powerful version of
\indtt{ground}\@.   It is essentially an iterated application of
\indtt{bddsimp} (whereas the propositional simplification in
\indtt{ground} is similar to that of \indtt{prop}),
\indtt{assert}, and \indtt{lift-if}\@.

The \emph{updates?} flag can be set to \texttt{NIL} in order
to avoid if-lifting update applications (see
page~\pageref{if-lift-updates})\@.   
\end{description}

\prsubsection{simplify}{Simplifying using decision procedures}

\begin{description}
\item[syntax:] \texttt{(simplify \optl\ \emph{fnums\default{*}} {\em
record?}\ \emph{rewrite?}\ \emph{rewrite-flag} \emph{flush?} {\em
linear?}\ {\em
  cases-rewrite?}\ \emph{type-constraints?\default{T}}
\emph{ignore-prover-output?})}

\item[effect:]
The  commands \indtt{assert}, \indtt{do-rewrite}, \indtt{record}, and
\indtt{simplify} used to all be primitive commands in PVS~1.x.  In PVS~2,
\indtt{simplify} is the only primitive command and
it is used in the definition of \indtt{assert}, \indtt{do-rewrite}, and {\tt
record}.  Thus \indtt{simplify} now includes  the arguments to 
\indtt{assert} along with two flags: \emph{record?}, and \emph{rewrite?}.
 For the  \indtt{assert} command,  \emph{record?}\ and \emph{rewrite?}\ 
must be \texttt{t}.  To get the \indtt{do-rewrite} command, \emph{record?}\ 
must be \texttt{
nil} and \emph{rewrite?}\ must be \texttt{t}.  To get the \indtt{record} command,
\emph{record?}\ must be \texttt{t}, and \emph{rewrite?}\ must be \texttt{nil}.
When \emph{record?}\ and \emph{rewrite?}\ are both \texttt{nil}, one
gets the effect of the \indtt{simplify} command of PVS~1.x.
...
(that is, a boolean expression) is
true or false (or not known to be either) with respect to the current
database and relative to theories such as those of equality over
uninterpreted function symbols and linear arithmetic.  In a sequent
of the form $\seq{a}{m}\vdash\seq{b}{n}$, the $a_i$ are simplified and
recorded as being true, and the $b_i$ are simplified and
recorded as being false.  The simplifications are described below.
The recording process can yield a refutation in
which case the sequent has been proved.

The theories handled by the ground decision procedures include:
\begin{enumerate}
\item The theory of equality with uninterpreted functions symbols.  This would
enable it to prove a sequent of the form \texttt{x = f(x) $\vdash$ f(f(f(x))) =
x}.
...
This ordering will affect certain PVS~1.x proofs since the forms of
certain expressions in the sequent will now be different following
simplification.  
...
\end{tabular}
The last two simplifications only happen when the type \texttt{T} is know
to be nonempty.
...
The current set of rewrite rules can be viewed using the
...
\item[notes:]  Unlike PVS~1.x, the \indtt{simplify} commands and their
variants can be placed within \indtt{repeat} strategies without the
danger of creating an infinitely looping strategy.
...
\prsubsection{auto-rewrite}{Installing Automatic Rewrite Rules}
\begin{description}

\item[syntax:] \texttt{(auto-rewrite \rest\ \emph{names})}

\item[effect:] The definitions, lemmas, and antecedent formulas
named (or numbered) in \emph{names} are 
made available for the descendant nodes of the current proof node as
automatic rewrite rules to be used for simplification by the {\tt
assert} rule.
Only formulas of a certain special form can be considered as automatic
rewrite rules.  An automatic rewrite rule is either:
\begin{itemize}
...
\item[errors:] Due to the presence of actual parameters, {\tt
auto-rewrite} can generate parsing and typechecking errors, in addition
...
\item[notes:] See the rule \indtt{stop-rewrite} for turning off an
automatic rewrite rule, \indtt{auto-rewrite-theory} for turning an entire
theory into rewrite rules, and \indtt{stop-rewrite-theory} to turn off
theory rewriting.  The Emacs command \emacstt{show-auto-rewrites}
displays the currently active rewrite rules in a separate buffer. 
The \indtt{track-rewrite} command can be used to explain why rewrite
rules did not perform the expected rewriting.  
A rewrite rule that is already installed is not affected by
\emacstt{modify-declaration} and might therefore need to be reinstalled.
\end{description}

\subsection[\texttt{auto-rewrite!/\char36}: Installing Eager Automatic Rewrite Rules]
{{\hugett auto-rewrite!/\char36:}Installing Eager Automatic Rewrite Rules}
...
\subsection[\texttt{auto-rewrite!!}: Installing Automatic Macros]
{{\hugett auto-rewrite!!:} Installing Automatic Macros}
\index{auto-rewrite"!"!@{\texttt{auto-rewrite"!"!}}|ii}
...
\prdolsubsection{auto-rewrite-defs}{Installing Relevant Definitions as Rewrites}

\begin{description}
\item[syntax:] \texttt{(auto-rewrite-defs \optl\ \emph{explicit?}\ \emph{always?}\ \emph{exclude-theories}) }
...
\prdolsubsection{auto-rewrite-explicit}{Installing Relevant Definitions as Rewrites}

\begin{description}
\item[syntax:] \texttt{(auto-rewrite-explicit \optl\  \emph{always?}) }
...
\prdolsubsection{auto-rewrite-theories}{Automatic Rewriting Relative to Theories}

\begin{description}

\item[syntax:] \texttt{(auto-rewrite-theories \rest\ \emph{theories})}
...
\prdolsubsection{auto-rewrite-theory}{Automatic Rewriting Relative to a Theory}

\begin{description}

\item[syntax:] \texttt{(auto-rewrite-theory \emph{name} \optl\ \emph{exclude} {\em
defs} \emph{always?}\ \emph{tccs?})}
...
\prdolsubsection{auto-rewrite-theory-with-importings}{Automatic Rewriting
Relative to a Theory and Its Importings}

\begin{description}

\item[syntax:] \texttt{(auto-rewrite-theory-with-importings \emph{name}
\optl\ \emph{exclude-theories} \emph{importchain?}\ \emph{exclude} \emph{defs} \emph{always?}\ \emph{tccs?})}
...
\prdolsubsection{install-rewrites}{Install Rewrite Rules from Names
and Theories}

\begin{description}
\item [syntax:]  \texttt{(install-rewrites \optl\ \emph{defs} {\em
theories} \emph{rewrites} \emph{exclude-theories} \emph{exclude})}
...
\prsubsection{stop-rewrite}{Turning Off Rewrite Rules}
\begin{description}

\item[syntax:] \texttt{(stop-rewrite \rest\ \emph{names})}

\item[effect:] Turns off those automatic rewrite rules named in {\em
names} that were turned on by either \indtt{auto-rewrite} or {\tt
auto-rewrite-theory}.

\item[usage:] \texttt{(stop-rewrite "append[nat]" "append[int]"
"length[nat]")} : Turns off automatic rewriting of these specific
rewrite rules regardless of whether they were added using {\tt
auto-rewrite} or \indtt{auto-rewrite-theory}.
...
\prdolsubsection{stop-rewrite-theory}{Turning Off Rewrite Rules from a Theory}

\begin{description}

\item[syntax:] \texttt{(stop-rewrite-theory \rest\ \emph{names})}

\item[effect:] Turns off any rewrite rules from the theories named in
\emph{names} that were turned on by either \indtt{auto-rewrite} or {\tt
auto-rewrite-theory}.  Any theory listed in \emph{names} must either be
...
\begin{tabular}{|l|l|l|}\hline
\indtt{typepred} & \emph{primitive} & introduces type constraints on
expressions\\\hline
\indttbang{typepred} & \emph{primitive} & minor variant of \indtt{typepred}\\\hline
\end{tabular}

\prsubsection{typepred}{Making Implicit Type Constraints Explicit}
\begin{description}

\item[syntax:] \texttt{(typepred \rest\ \emph{exprs})}
...
decision procedures in most cases; this command is only occasionally
needed.
...
\item[syntax:] \texttt{(typepred!\ \emph{exprs} \optl\ \emph{all?})}
...
all the type predicates for subtypes of number.  The
...
\begin{tabular}{|l|l|l|}\hline
\indtt{abstract-and-mc} & \emph{defined} & Boolean/data abstraction followed by model-checking\\\hline
\indtt{abstract} & \emph{defined} &  Boolean abstraction of PVS expressions\\\hline
\indtt{abs-simp} & \emph{primitive} & Boolean abstraction \\\hline
\indtt{model-check} & \emph{defined} &  CTL model checking\\\hline
\indtt{musimp} & \emph{primitive} & mu-calculus model checker \\\hline
\end{tabular}

\prdolsubsection{abstract-and-mc}{Boolean abstraction followed by model-checking}
\begin{description}
...
\emph{rewrites}
\emph{exclude}
\emph{strategy}\default{\texttt{(assert)}}
\emph{feasible}
\emph{verbose?}
)}
...
\prdolsubsection{abstract}{Abstraction of PVS expressions}
\begin{description}
...
\prdolsubsection{abs-simp}{Boolean abstraction}
\begin{description}
...
\prdolsubsection{model-check}{CTL Model Checking}

\begin{description}
\item[syntax:]  \texttt{(model-check \optl\ {\em
 dynamic-ordering?}\default{T}
\emph{cases-rewrite?}\default{T}
\emph{defs}
\emph{theories} \emph{rewrites}
\emph{exclude} \emph{irredundant?})}
...
The commands \emph{defs}, \emph{theories}, \emph{rewrites}, and {\em
exclude}, are used exactly as in \indtt{install-rewrites} to set
...
\begin{usage}{}
\item[\texttt{(model-check :theories ("transitions" "properties")
...
\prsubsection{musimp}{Mu-Calculus Model Checking}

\begin{description}
\item[syntax:]  \texttt{(musimp \optl\ \emph{fnums}\default{*} {\em
dynamic-ordering?}\default{NIL})}

\item[effect:]  This command is primarily used in the \indtt{model-check}
strategy to invoke the BDD-based, symbolic mu-calculus model checker.
A glaring weakness of this model checker is that it does not generate
a counterexample trace.  The outcome of the command is usually a collection of
subgoals corresponding to the states that violate the mu-calculus formula
given by the sequent formulas selected using \emph{fnums}\@.
...
\begin{tabular}{|l|l|l|}\hline
\indtt{apply} & \emph{primitive} & applies a proof strategy in a single atomic step\\\hline
\end{tabular}


\prsubsection{apply}{ Making Proof Strategies Atomic}
\begin{description}
\item[syntax:] \texttt{(apply \emph{strategy}\ \optl\ \emph{comment}
\emph{save?} \emph{time?})}

\item[effect:] The \indtt{apply} rule takes an application of a proof {\em
strategy} and applies it as a single atomic step that generates those
subgoals left unproved by the proof strategy.  The \indtt{apply} rule is
frequently used when one wishes to employ a proof strategy but is not
interested in the details of the intermediate steps.  A number of
defined rules employ \indtt{apply} to suppress trivial details.
The optional \emph{comment} field can be used to provide a
format string to be used as commentary while printing out the proof.
If the \texttt{save?} flag is set to \texttt{T}, the \texttt{apply} step is
saved even if the applied strategy results in no change to the
proof.  This is useful if, for example, the command within the apply
uses the \texttt{lisp} command to change a Lisp variable for use elsewhere in
the proof.  The \texttt{time?} flag when \texttt{T} is used to return timing
...
We describe the more easily understood aspects of strategies below.
Any strategy expression can be typed in at the \texttt{Rule?}\ prompt
in a proof.  The
syntax for strategy expressions is as follows:
...
\pair{step} &:= & \pair{\textit{primitive-rule}}\\
           ...
           & | & \texttt{(if}\ \pair{\textit{lisp-expression}}\ \pair{\textit{step}}\
\pair{\textit{step}}\texttt{)}\\
           & | & \texttt{(let\ (}\{{\tt(} \pair{\textit{symbol}}\ \pair{\mbox{\em
lisp-expression}}\texttt{)}\}^+\texttt{)}\ \pair{\textit{step}}\texttt{)}
\end{eqnarray*}

\section{Defining Strategies}

User-defined strategies should be saved in a file called \indtt{pvs-strategies}.
\pvs\ loads strategies from files of this name from
both the  user's home directory and the current context directory.
...
This generates both a (blackbox) defined rule  \emph{name} and a
(glassbox) strategy
\emph{
name\texttt{\char36}}.  There are two other definition forms that are
essentially similar to \indtt{defstep}.  These are \indtt{defstrat} and
\indtt{defhelper}\@.  The \indtt{defhelper} version is identical to
\indtt{defstep} 
but is used for defining strategies that are only meant to be used in the
definition of other strategies and are not likely to be invoked directly by
the user.  The \indtt{defstrat} version defines only a glassbox strategy
called \emph{name} and the rule or blackbox version is not defined.
The \indtt{defstrat} form does not take the final format-string argument
given to \indtt{defstep}\@.  
The differences between a defined rule and a strategy are:
\begin{enumerate}
...
strategy returns the expanded proof tree.  For example, {\tt
prop\char36} and \texttt{ground} are strategies and \indtt{prop} and {\tt
ground} are their corresponding rule versions.  The former are glass
...
\item[syntax:] \texttt{(quote \emph{step})}

\item[effect:]  The strategy expression  \texttt{(quote \emph{step})}
simply evaluates to \emph{step} itself.  The reason it is needed is
that it is quite typical to construct a Lisp expression corresponding
to a strategy but this then needs to be unquoted to be used as a
strategy.  For example the strategy \texttt{(then* \rest\ steps)}
is given the recursive definition
\begin{alltt}
  (let ((x (when steps (car steps)))
        (y (when steps (cons 'then* (cdr steps)))))
    (if steps (if (cdr steps)(then x y) x) (skip)))
\end{alltt}
In evaluating the body of this definition, the quoted forms of the
values of the bindings for \texttt{x}, \texttt{y} and \texttt{z} are
substituted for these variables into the body of the let-expression.
When any of these values are to be evaluated as strategies, they
are unquoted by the definition for the \indtt{quote} strategy given
above.
\end{description}
...
\item[syntax:] \texttt{(try \emph{step1} \emph{step2} \emph{step3})}

\item[effect:] This is the basic control strategy.  It applies {\em
step1} to the current goal.  If \emph{step1} succeeds and generates
subgoals, then \emph{step2} is applied to those subgoals.  If {\em
step1} does nothing, \ie\ behaves as a \texttt{(skip)} step, then {\em
step3} is applied to the current goal.  The \indtt{try} step thus
provides a backtracking mechanism for proof search that can be
controlled by appropriately signalling failure.  The following
identities describe the behavior of the \indtt{try} strategy:
...
to be closed.  Then \texttt{(fail)} backtracks to the last open
(\ie\ not closed) alternative.
...
\item[syntax:] \texttt{(if \emph{condition} \emph{step1} \emph{step2})}
...
\item[usage:] \texttt{(if (equal (get-goalnum *ps*) 1)
(ground) (prop))} : If the current goal (\texttt{*ps*} is the current
proofstate) is the first subgoal of its parent, the apply {\tt
(ground)}, else apply \texttt{(prop)}.

\end{description}


\prsubsection{let}{Using Lisp in Strategies}
\begin{description}

\item[syntax:] \texttt{(let ((\emph{var$_1$} \emph{lexpr$_1$}) $\ldots$
(\emph{var$_n$} \emph{lexpr$_n$})) \emph{step})}
...
The \indtt{then*} strategy has the definition shown below where it
is defined by iterating the \indtt{then} strategy.
{\smaller
\begin{alltt}
  (then* \&rest steps)
     = (let ((x (when steps (car steps)))
...
\prsubsection{branch}{Assigning Strategies to Subgoals}
\begin{description}

\item[syntax:] \texttt{(branch \emph{step} \emph{steplist})}

\item[effect:] Just like spread except that when \emph{steplist} has
only $n$ elements and \emph{step} generates more than $n$ subgoals, the
$n$'th element of \texttt{steplist} is also applied to the subgoals
following the $n$'th one.  This is useful when only the first few
subgoals generated by \emph{step} require special attention and the
rest of the subgoals yield to some uniform strategy.
\end{description}
...
\item[syntax:] \texttt{(else \emph{step1} \emph{step2})}
...
\item[effect:] A synonym for \texttt{query*}: inserting \texttt{(checkpoint)}
into an edited proof and rerunning it causes the non-checkpointed
subproofs to be installed (using \texttt{just-install-proof}) so that the
proof is only run up to the checkpoint.

\item[notes:] This command is not meant to be used directly; see the User
Guide for using it in editing proofs. 
\end{description}


\prsubsection{just-install-proof}{Checkpoint Handling}
\begin{description}

\item[syntax:] \texttt{(just-install-proof \em{proof})}
...
\item[syntax:] \texttt{(repeat \emph{step})}
...
repetition is continued along each branch until an application of {\em
step} has no effect.
\end{description}
...
\item[syntax:] \texttt{(rerun \optl\ \emph{proof})}

\item[effect:] This step can be used to rerun a partial or completed
proof from a previous attempt or from another branch of the proof.  This
step is largely used automatically by the system when it queries as to
whether the proof should be rerun.  The \emph{proof} argument can also
be explicitly given by the user using either the \emacstt{edit-proof} or
\emacstt{show-proof} commands to generate and edit such inputs.  This
step can be  used to:
\begin{enumerate}
...
\prsubsection{spread}{Assigning Strategies to Subgoals}
\begin{description}

\item[syntax:] \texttt{(spread \emph{step} \emph{steplist})}

\item[effect:] First applies \emph{step1} and then applies the $i$'th
element of \emph{steplist} to the $i$'th subgoal.  This is typically
used when \emph{step} splits the proof into multiple branches where a
different strategy is required for each of the branches.  See {\em
branch} below.
\end{description}
...
\item[effect:] Like \indtt{spread}, applies \emph{step} and then pairs
the steps in \emph{steplist} with 
...
\item[effect:] Like \indtt{spread}, applies \emph{step} and then pairs
the steps in \emph{steplist} with 
the subgoals, but generates a warning if the number
of subgoals do not match the number of subproofs.
\end{description}
...
\item[syntax:] \texttt{(then \rest\  \emph{steps})}

\item[effect:] Let the list \emph{steps} consists of the first
element \emph{step1} and the remaining steps \emph{rest-steps}\@. 
This strategy first applies the \emph{step1} to the current goal.  If any
subgoals are 
generated, then  \texttt{(then :steps \emph{rest-steps})} is applied to each
of these subgoals.  If 
\emph{step1} has no effect, then \texttt{(then :steps \emph{rest-steps})}
is applied to the original goal.  The definition of \indtt{then} is just
\texttt{(try \emph{step1} \emph{step2} \emph{step2})}.
\end{description}


\prsubsection{then*}{Apply Steps in Sequence}

\begin{description}

\item[syntax:] \texttt{(then* \rest\ \emph{steps})}

\item[effect:] This command is subsumed by \indtt{then} and should now be
considered obsolete.  This is an iterated version of \indtt{then} so that
\texttt{(then* \emph{step1} \ldots \emph{stepn})} is just \texttt{(then step1
(then \ldots \emph{stepn}))}.
\end{description}
...
\item[effect:] This is a version of \indtt{then*} where the given steps
are applied in sequence only along the main branch of the proof, \ie\ if
the the given rule is \texttt{(then@ \emph{step$_1$} \ldots {\em
step$_n$})}, then \emph{step$_{i+1}$} is only applied to the first
subgoal of \emph{step$_i$}.
\end{description}


\prsubsection{time}{Time a Given Strategy}
\begin{description}
\item[syntax:] \texttt{(time \emph{strategy})}
...
\item[syntax:] \texttt{(try-branch \emph{step1} \emph{steplist} \emph{step2})}
...
\bibliography{/project/pvs/doc/pvs,/homes/rushby/jmr,/homes/shankar/tex}
