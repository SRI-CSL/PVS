...
\def\optl{{\smaller\sc{\&}optional}}
\def\rest{{\smaller\sc{\&}rest}}
...
Rule? (lemma "nat_induction")\indskobang\indextt{lemma}
...
\begin{pvsex}
Rule? (inst - "(LAMBDA n: sum((LAMBDA (n: nat): f!1(n) + g!1(n)), n)\indextt{inst}
          = sum(f!1, n) + sum(g!1, n))")
...
\begin{pvsex}
Rule? (expand "sum")\indextt{expand}
...
It is useful to have the branching operation \indtt{IF} in the language
for expressing conditional expressions.  For each type $\alpha$, there is
an \indtt{IF} operation with the signature
$[\texttt{bool}, \alpha, \alpha \rightarrow \alpha]$.  The transformation
of $A[e]$ to $A[b]$ represents the replacement of the highlighted
occurrences of $e$ in $A$ by $b$.  Note that for the
${\indtt{IF}\uparrow\:\vdash}$ and ${\vdash\:\texttt{IF}\uparrow}$ rules,
the $A$ in $B[\texttt{IF}(A, b, c)]$ must not contain any free variable
occurrences that are bound in $B[\texttt{IF}(A, b, c)]$.  The inference
rules for \indtt{IF} are:
...
\begin{center}
  \texttt{(\textrm{\emph{\bkt{rulename} \bkt{required}$^{\textstyle *}$
...
\texttt{-3} by the label \texttt{unique\-ness}.

\item[\texttt{(label type-constraints (-1 -3 -4))} :] Labels the formulas
numbered \texttt{-1}, \mbox{\texttt{-3}}, and \texttt{-4} by the label
...
\item[\texttt{(bddsimp type-constraints)} :] Does BDD-based
...
\item[effect:] Terminates the current proof attempt, and asks whether
...
\item[effect:] Has no effect on the proof but prints the \emph{msg}
...
Tracks the given re\-write rules during 
...
labeled \texttt{assoc.2}, unless \texttt{assoc.2} labels the current node in
...
combination is used in the defined rule \indtt{case-replace}
...
\item[notes:]
\begin{itemize}
\item This command causes branching and should be used with
caution; otherwise you will end up doing essentially the same
...
\item[effect:] A top-level sequent formula of the form {\smaller\texttt{(FORALL
(x:\ [S1, S2]):\ g(x))}} or {\smaller\texttt{(FORALL (x:\ [\# s:\ S, t:\ T \#]):\
g(x))}} is replaced by {\smaller\texttt{(FORALL (x1:\ S1), (x2:\ S2):\ g(x1, x2))}}
and {\smaller\texttt{(FORALL (x1:\ S), (x2:\ T)):\ g((\# s := x1, t := x2 \#)))}}\@.
...
{\smaller\texttt{$\vdash$ (FORALL x:~(a1(x) AND a2(x)) IMPLIES (c1(x) OR c2(x)))}}\@.
...
universally quantified antece\-dent formula with exactly
...
and replaces it with $A[\texttt{x + 3}/x_1, \texttt{y - z}/x_2]$.

\item[\texttt{(instantiate - ("x + 3" "y - z"))}] : Searches for the first suitable
antece\-dent formula in the current sequent, and has the same effect as
...
\item[{\bf No suitable quantified formula found}:] No formula
of existential strength was found in the range given by \emph{fnum} with the same
...
sneak in through the \indtt{instanti\-ate} rule.
...
\item[effect:] If the formula in the current sequent indicated by
  \emph{fnum} is either an antecedent formula of the form
  $(\exists x_1,\ldots, x_n : A)$ or a consequent formula of the form
  $(\forall x_1, \ldots, x_n : A)$, and \emph{constants} is a list of new
  identifiers of the form \texttt{($c_1$ \ldots $c_n$)}, then the
  \indtt{skolem} rule generates a subgoal where the formula has
  been replaced by $A[c_1/x_1,\ldots, c_n/x_n]$.\footnote{This is slightly
    inaccurate since in PVS, the type for the bound variable $x_i$ in
    $(\forall x_1, \ldots, x_n : A)$ can contain free occurrences of $x_j$
    where $j<i$ which is not reflected by the substitution instance
    $A[c_1/x_1,\ldots, c_n/x_n]$.}  When \emph{fnum} is \texttt{*}, then
  the first appropriate sequent formula (\ie\ either an antecedent
  existential formula or a consequent universal formula binding $n$
  variables) is chosen for quantifier elimination.  The parameter
  \emph{fnum} can also be either \texttt{+} or \texttt{-}.  If any of the
  $c_i$ are given as \texttt{"\_"}, then those bound variables are left
  alone and no corresponding skolem constants are introduced.  If
  \emph{skolem-typepreds?}\ is \texttt{t}, then typepreds will be
  introduced for the new constants.
...
uses \indtt{assert} instead of \indtt{simpli\-fy} in the simplification process. 
\end{usage}
...
associativity of \texttt{app\-end} for lists of natural numbers to the
...
lemma \texttt{assoc} in a leftmost-outermost order and re\-writes
...
\texttt{:rewrites "assoc" :exclude ("div\_times" "add\_div"))}:
...
with two flags: \emph{re\-cord?}, and \emph{rewrite?}.  For the
...
implication, e.g. $H \imp l = r$, or the condition of an $IF$ or
$CASES$ expression.)
...
\item[{\bf Can't rewrite using \ldots: LHS key \ldots is bad.} :]
  ...
  is the left-most operator name or expression type such as a record
  constructor, tuple constructor, update expression, tuple projection,
  record field access, cases expression, and lambda, forall, and exists
  expressions.  Note that having too many rewrite rules attached to a
  single key can slow down rewrite lookup.

\item[{\bf RHS free variables must be contained in the LHS free
variables} :] The matching for rewriting is done using the left-hand
side of the rewrite rule, so there should be no free variables left in the
...
numbers, then the antecedent formula \mbox{\texttt{i + j >= 0}} is added to the
current sequent along with the assertions \newline
\mbox{\texttt{integer\_pred(i + j)}},
\texttt{rational\_pred(i + j)}, and \texttt{real\_pred(i + j)}.

\item[\texttt{(typepred "cons(i, null)")}] : Adds the antecedent formula \newline
...
\indtt{assert} to apply these re\-writes and other simplifications,
...
\item[\texttt{(model-check :theories ("transitions" "props")
    :irredundant?\ T)} :] \mbox{\hspace{1em}}
  Translates the given sequent containing CTL or mu-calculus assertions
  into a Boolean mu-calculus, invokes a BDD-based symbolic model checker
  on this, and either proves the result or returns a collection of
  subgoals.
...
step succeeds):
{\smaller\begin{enumerate}\label{trypage}
...
= (flatten)}
\item \texttt{(try (try (lemma "assoc") (assert) (fail)) (split)
    (flatten)) \newline
...
\item[{\smaller\texttt{(try (flatten) (propax) (split))}}] : Applies the disjunctive
...
\item[{\smaller\texttt{(try (try (flatten) (fail) (skolem 1 ("a" "b"))) (postpone)
