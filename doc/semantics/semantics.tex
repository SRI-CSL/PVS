...
\usepackage{fleqn,alltt,spacecites,relsize,url,amssymb}
...
\comment{
\memo{I am not exactly sure where the axiom of choice is really needed.
...
constructed as $\tupletype{\listwo{A}{n}}$.  \comment{We make no
distinction between a type $A$ and the singleton tuple type $[A]$.  We
also disallow a 0-tuple type.}  A \emph{type} is a pretype that has been
...
$\proj{i}\ a$, where $i\in\{1, 2\}$\@.  \comment{As with types, we make no
distinction between the singleton tuple preterm $(a)$ and the preterm
$a$. }  Lambda abstractions have the form $\lambda (x : T): a$, where $T$
is a pretype and $a$ is a preterm.  \comment{ $\lambda (x_1 : T_1, \ldots,
x_n : T_n): a$, where each $T_i$ is a type, each $x_i$ is a symbol so that
$x_i\not\equiv x_j$ for $i\neq j$, and $a$ is a preterm. }  Parentheses
...
\comment{
\memo{The above definition suggests that projection applications should
...
\comment{The base types 
\texttt{bool} and \texttt{real} are mapped to $\twob$ and $\reals$,
respectively.}  The meanings of type names, constants, and variables
...
Cartesian product. \comment{an $n$-tuple is represented by an $(n+1)$-fold
...
\comment{We will also use the notation $\lambda (x_1 : T_1, \ldots, x_n : T_n): a$
...
\comment{With subtypes, it is easy to define an empty subtype and then
...
\comment{Theorems~\ref{simple-type-semantics}
and~\ref{simple-term-semantics} must be proved with a simultaneous
induction since types now contain terms.}  The statement of
...
\comment{Let $V$ be a metavariable that ranges over types or {\em
type bindings\/} of the form $x : T$.}  A dependent product type is
...
\comment{ To determine whether a given expression is of a given dependent type, we
...
\comment{ A simpler alternative might be to define a second universe
...
\comment{ Seeking advice on PVS semantics:  I'd like to know if the following
...
\comment{\memo{Perhaps each declaration should be required to introduce a new
...
\comment{The definition of $\delta$ has to be modified slightly to carry along the
...
\comment{This map preserves the
...
\comment{\memo{Note: no free variables in a, b.  Need to show that
...
\newpage
\bibliographystyle{/homes/rushby/tex/modalpha} % or modplain
