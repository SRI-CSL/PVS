...
The third and final theory in file \texttt{stack\_adt.pvs} generated from
...
% \item A disjointness axiom is generated that says that no element of the
% datatype can belong to more than one recognizer subtype.
...
%% The reason for the accessor subtype restriction is illustrated by the
%% following datatype.
%% \begin{pvsex}
%%   d[T: TYPE]: DATATYPE
%%    b(n: nat): b?
%%    c(a: {x: d | b?(x) AND EXISTS (a, b: T): a /= b}): c?
%%   END d
%% \end{pvsex}
%% Now the generated map theory would have two type parameters, \texttt{T}
%% and \texttt{T1}, and the definition for \texttt{map} should map
%% \texttt{c?[T]} elements to \texttt{c?[T1]} elements, but when instantiated
%% as \texttt{map[nat,{x:nat | x = 0}]} there is nothing to map the
%% \texttt{c?[T]} elements to.  Although this is an artificial example, it is
%% not obvious 
...
Like record types, constructor selectors may be dependent
