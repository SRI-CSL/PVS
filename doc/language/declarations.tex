...
\index{exporting|(}\index{importing|(} Declarations introduced in one
...
PVS the kinds are \emph{type}\index{declaration!kind!type},
...
g, b\}} \index{enumeration types}\index{type!enumeration}
...
it is nonempty it generates an \emph{existence TCC}.\index{existence
...
of nonemptiness, and \keyword{CONTAINING}\index{CONTAINING@\texttt{CONTAINING}} clauses on subtypes, as well as
...
\item Any interpreted subtype declaration with a \keyword{CONTAINING}
clause is marked nonempty, and the \keyword{CONTAINING} expression is
typechecked against the specified type.  In this case no existence TCC is
generated, since the \keyword{CONTAINING} expression is a witness to the
type.  Of course, other TCCs may be generated as a result of typechecking
the \keyword{CONTAINING} expression.
...
\texttt{POSTULATE}\index{postulate}.  In the prelude postulates are used
