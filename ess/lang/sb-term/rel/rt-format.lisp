...
(defparameter def-indent-unit-width (sb-chars-to-pixels indent-unit))
...
(defparameter *rept-indent-units* (* 1 indent-unit))
(defparameter *uterm-indent-units* (* 2 indent-unit))
  ...
  (cond ((and bp new-bp)
	 (setf (bp-value bp)
	       (cond ((and (bp-value bp) (bp-value new-bp))
		      (min (bp-value bp) (bp-value new-bp)))
		     (t
  ...
  (if (no-tabs-or-indents (tinfo-bp-format tinfo))
      (let* ((new-indents 
	     ...
	     (new-unindents (cond ((tinfo-unindents tinfo))
				  (t 0)))
	     (indents-to-add (- new-indents new-unindents)))
	;;(format t "~A: ~A~%" 
	;;  (tinfo-index tinfo)
...
(defun assign-between-token-whitespace ()
  (do ((i 0 (1+ i)))
    ...
    (let* ((tinfo (aref *tinfo* i))
	  ...
	  (setf (bp-crs bp) (if (= 0 (bp-spaces bp)) 0 1)))
...
(defun setup-bp-tree ()
  (let ((root (aref *tinfo* 0))
	...
	(set-width new)
	(cond ((>= (tinfo-bp-value new)
		   (tinfo-bp-value curr-att))
...
(defun balance-bp-tree (root start end)
  (if root
	...
	(cond ((tinfo-rson root)
	       (cond ((= (tinfo-bp-value root)
			 (tinfo-bp-value (tinfo-rson root)))
		      (do ((i 1 (1+ i))
			   (rson (tinfo-rson root) (tinfo-rson rson))
			   (best-rson root
				      (if (< (abs (- (tinfo-width rson)
						     mid-point))
					     (abs (- (tinfo-width best-rson)
						     mid-point)))
					  rson
					  best-rson)))
...
(defun should-break-intv? (start end)
  (or (> (+ (- (tinfo-width (aref *tinfo* end))
	       (if (>= start 1)
		   (tinfo-width (aref *tinfo* (1- start)))
		   0))
	    (if (>= start 1)
		(cond ((car (tinfo-indent-stack (aref *tinfo* (1- start)))))
		      (t 0))
		0))			; indentation!
	 *allowed-width*)))
...
(defun format-uterm (uterm width
  ...
  (declare (ignore top-width bottom-width))
  (let* ((*sb-fontwidth* fontwidth)
	 ...
	 (aw (memo-aw uterm
		      ...
		      *indent-unit-width*
		      ...
		      fontheight)))
    (let ((aw
	   (cond ((and (> (aw-maxwidth aw) width)
		       (= (aw-indent-unit-width aw) 0))
...
(defun oct-points (oct y-offset height)
  ...
  ; added to y-coordinates.
    (let ((x1 (- (leftx oct) 1))
	  ...
	  (x3 (rightx oct))
	  (x4 (botdent oct))
	  (y1 (+ (topy oct) height y-offset))
	  (y2 (+ (topy oct) y-offset))
	  (y3 (+ (- (boty oct) height) y-offset))
	  (y4 (+ (boty oct) y-offset)))
      (list (list x1 y1)
	    (list x2 y1)
	    (list x2 y2)
	    (list x3 y2)
